...
function X() {
  const M = {
        ...
        (this.object = {
          ...
          _finalize: function (t) {
            ...
            if (
              (i &&
                i.groupEnd === -1 &&
                ((i.groupEnd = this.geometry.vertices.length / 3),
    ...
    addVertex: function (s, e, n) {
      ...
      (i.push(t[s + 0], t[s + 1], t[s + 2]),
        i.push(t[e + 0], t[e + 1], t[e + 2]),
        i.push(t[n + 0], t[n + 1], t[n + 2]));
    },
    ...
    addColor: function (s, e, n) {
      ...
      (t[s] !== void 0 && i.push(t[s + 0], t[s + 1], t[s + 2]),
        t[e] !== void 0 && i.push(t[e + 0], t[e + 1], t[e + 2]),
        t[n] !== void 0 && i.push(t[n + 0], t[n + 1], t[n + 2]));
    },
    ...
    addFace: function (s, e, n, t, i, a, d, g, r) {
      ...
      let o = this.parseVertexIndex(s, h),
        l = this.parseVertexIndex(e, h),
        u = this.parseVertexIndex(n, h);
      if ((this.addVertex(o, l, u), this.addColor(o, l, u), d !== void 0 && d !== '')) {
        const m = this.normals.length;
        ((o = this.parseNormalIndex(d, m)),
          (l = this.parseNormalIndex(g, m)),
          (u = this.parseNormalIndex(r, m)),
          this.addNormal(o, l, u));
      } else this.addFaceNormal(o, l, u);
      if (t !== void 0 && t !== '') {
        const m = this.uvs.length;
        ((o = this.parseUVIndex(t, m)),
          (l = this.parseUVIndex(i, m)),
          (u = this.parseUVIndex(a, m)),
          this.addUV(o, l, u),
    ...
    addLineGeometry: function (s, e) {
      ...
      const n = this.vertices.length,
        t = this.uvs.length;
      for (let i = 0, a = s.length; i < a; i++) this.addVertexLine(this.parseVertexIndex(s[i], n));
      for (let i = 0, a = e.length; i < a; i++) this.addUVLine(this.parseUVIndex(e[i], t));
    },
  };
  return (M.startObject('', !1), M);
}
class te extends N {
  constructor(s) {
...
`);
    ...
    for (let d = 0, g = n.length; d < g; d++) {
      ...
      if (h !== '#')
        if (h === 'v') {
          ...
          switch (o[0]) {
            case 'v':
              (e.vertices.push(parseFloat(o[1]), parseFloat(o[2]), parseFloat(o[3])),
                o.length >= 7
                  ? (_.setRGB(parseFloat(o[4]), parseFloat(o[5]), parseFloat(o[6]), T),
                    e.colors.push(_.r, _.g, _.b))
                  : e.colors.push(void 0, void 0, void 0));
              break;
            case 'vn':
              e.normals.push(parseFloat(o[1]), parseFloat(o[2]), parseFloat(o[3]));
              break;
            case 'vt':
              e.uvs.push(parseFloat(o[1]), parseFloat(o[2]));
              break;
          }
        } else if (h === 'f') {
          const l = r.slice(1).trim().split(D),
            u = [];
          ...
          for (let c = 1, p = u.length - 1; c < p; c++) {
            const b = u[c],
              v = u[c + 1];
            e.addFace(m[0], b[0], v[0], m[1], b[1], v[1], m[2], b[2], v[2]);
          }
        } else if (h === 'l') {
          ...
          e.startObject(o);
        } else if (K.test(r)) e.object.startMaterial(r.substring(7).trim(), e.materialLibraries);
        else if (W.test(r)) e.materialLibraries.push(r.substring(7).trim());
        else if (Q.test(r))
          console.warn(
    ...
    if (
      ...
      for (let d = 0, g = e.objects.length; d < g; d++) {
        ...
        const c = new A();
        (c.setAttribute('position', new w(h.vertices, 3)),
          h.normals.length > 0 && c.setAttribute('normal', new w(h.normals, 3)),
          h.colors.length > 0 && ((m = !0), c.setAttribute('color', new w(h.colors, 3))),
          h.hasUVIndices === !0 && c.setAttribute('uv', new w(h.uvs, 2)));
        const p = [];
        for (let v = 0, I = o.length; v < I; v++) {
          const y = o[v],
            P = y.name + '_' + y.smooth + '_' + m;
          let f = e.materials[P];
          ...
          (f === void 0 &&
            (l ? (f = new C()) : u ? (f = new L({ size: 1, sizeAttenuation: !1 })) : (f = new J()),
            (f.name = y.name),
            (f.flatShading = !y.smooth),
            (f.vertexColors = m),
            (e.materials[P] = f)),
            p.push(f));
        }
        ...
        if (p.length > 1) {
          ...
          l ? (b = new S(c, p)) : u ? (b = new F(c, p)) : (b = new B(c, p));
        } else l ? (b = new S(c, p[0])) : u ? (b = new F(c, p[0])) : (b = new B(c, p[0]));
        ((b.name = r.name), i.add(b));
      }
    else if (e.vertices.length > 0) {
      const d = new L({ size: 1, sizeAttenuation: !1 }),
        g = new A();
      (g.setAttribute('position', new w(e.vertices, 3)),
        e.colors.length > 0 &&
          e.colors[0] !== void 0 &&
          (g.setAttribute('color', new w(e.colors, 3)), (d.vertexColors = !0)));
      const r = new F(g, d);
      i.add(r);
    }
    return i;
