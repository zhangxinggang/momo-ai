import { p as re } from './chunk-4BX2VUAB-T0fy3fOG.js';
import { s as ve } from './markdown-it-vendor-DL4wSELR.js';
...
import { i as be, p as pe } from './wardley-L42UT6IY-PPlg582g.js';
var T = 'position frame',
  $ = 'frame positioned',
  S = 'position relation',
...
o(N, 'reset');
var Se = ge.eventmodeling,
  ke = o(() => fe({ ...Se, ...M().eventmodeling }), 'getConfig'),
  B = {};
function O() {
  let e = Fe;
  const { ast: n } = B,
  ...
  return (
    n.frames.forEach((i, a) => {
      const r = V(i, n.dataEntities, t);
      e = v(e, { $kind: T, index: a, frame: i, textProps: r });
      let d;
      K(i)
        ? (g.debug('source frame', i.sourceFrames),
          (d = n.frames.filter((l) => i.sourceFrames.some((c) => c.$refText === l.name))),
          d.forEach((l) => {
            e = v(e, { $kind: S, index: a, frame: i, sourceFrame: l });
          }))
        : (e = v(e, { $kind: S, index: a, frame: i }));
    }),
    (e = { ...e, sortedSwimlanesArray: A(e.swimlanes) }),
    e
  );
}
o(O, 'getState');
  ...
  switch (e.modelEntityType) {
    ...
    case 'processor':
      return i
        ...
        : t
          ? { index: b(n, 0, 100), label: s.labelUiAutomationPrefix + t }
    ...
    case 'command':
      return i
        ...
        : t
          ? { index: b(n, 100, 200), label: s.labelCommandReadModelPrefix + t }
          : { index: 100, label: s.labelCommandReadModel };
    default:
      return i
        ? { index: i.index, label: i.namespace || s.labelEvents }
        : t
          ? { index: b(n, 200, 300), label: s.labelEventsPrefix + t }
...
function L(e) {
  ...
  switch (e.modelEntityType) {
    case 'ui':
      return { fill: n.emUiFill ?? 'white', stroke: n.emUiStroke ?? '#dbdada' };
    case 'pcr':
    case 'processor':
      return { fill: n.emProcessorFill ?? '#edb3f6', stroke: n.emProcessorStroke ?? '#b88cbf' };
    case 'rmo':
    case 'readmodel':
      return { fill: n.emReadModelFill ?? '#d3f1a2', stroke: n.emReadModelStroke ?? '#a3b732' };
    case 'cmd':
    case 'command':
      return { fill: n.emCommandFill ?? '#bcd6fe', stroke: n.emCommandStroke ?? '#679ac3' };
    case 'evt':
    case 'event':
      return { fill: n.emEventFill ?? '#ffb778', stroke: n.emEventStroke ?? '#c19a0f' };
    default:
...
function V(e, n, t) {
  ...
  if (
    (e.dataInlineValue &&
      ...
      (r = y(r, i)),
      (r = P(r, t.textMaxWidth, d)),
      (r = r.replaceAll(' ', '&nbsp;'))),
    e.dataReference)
  ) {
    const p = n.find((w) => w.name === e.dataReference?.$refText);
    p &&
      ((r = p.dataBlockValue),
      (r = r.substring(
        r.indexOf(`{
`) + 2,
  ...
  const m = r !== void 0;
  m &&
    (c += `<br/><br/><code style="text-align: left; display: block;max-width:${t.textMaxWidth}px">${r}</code>`);
  const x = { fontSize: d.fontSize, fontWeight: d.fontWeight, fontFamily: d.fontFamily },
    u = he(c, x),
    h = m ? u.width / 3 : u.width,
    f = { content: c, width: h, height: u.height };
  return (g.debug(`[${e.name}] ${e.entityIdentifier} text`, f), f);
}
o(V, 'calculateTextProps');
function G(e, n) {
  const t = n,
    i = L(t.frame),
    a = {
  ...
  return [
    { $kind: $, frame: t.frame, index: t.index, visual: i, dimension: a, textProps: t.textProps },
  ];
}
...
function Y(e, n) {
  ...
  const r = e.boxes.length > 0 ? e.boxes[e.boxes.length - 1] : void 0,
    ...
    l = {
      ...
      height:
        Math.max(s.boxMinHeight, Math.min(s.boxMaxHeight, t.dimension.height)) + 2 * s.boxPadding,
    },
    c = X(a, d, r),
    m = c + l.width + s.boxPadding,
    x = j(Object.values(e.swimlanes), m);
  ((a.r = c + l.width),
...
function J(e, n) {
  const t = n;
  if (be(t.frame) || z(t.index, t.frame)) return [];
  const i = k(e.boxes, t.frame);
  if (i === void 0) throw new Error(`Target box not found for frame ${t.frame.name}`);
  let a;
  return (
    t.sourceFrame
      ? (a = k(e.boxes, t.sourceFrame))
      : (a = q(e.boxes, i.swimlane.index, t.index - 1)),
...
function ie(e, n, t, i) {
  return (a) => {
    ...
    let x, u;
    (g.debug(`rendering relation up=${l} for `, { sourceBox: a.sourceBox, targetBox: a.targetBox }),
      l
        ? ((x = r), (u = d + a.targetBox.dimension.height))
        : ((x = r + a.sourceBox.dimension.height), (u = d)));
...
function ae(e, n, t, i) {
  return (a) => {
    const r = e.append('g').attr('class', 'em-swimlane'),
      d = i.emSwimlaneBackgroundOdd ?? 'rgb(250,250,250)',
      l = i.emSwimlaneBackgroundStroke ?? 'rgb(240,240,240)';
    (r
      .append('rect')
      .attr('x', 0)
      .attr('y', a.y)
...
`,
    ...
    const a = i.db,
      ...
      m = a.getState(),
      x = `em-arrowhead-${n}`,
      u = r.emArrowhead ?? '#000000';
    (m.sortedSwimlanesArray.forEach(ae(l, m.maxR, c, r)),
      m.boxes.forEach(te(l, c)),
      m.relations.forEach(ie(l, c, x, r)),
      l
        .append('defs')
        .append('marker')
        .attr('id', x)
        ...
        .attr('fill', u),
      ue(void 0, l, d?.padding ?? 30, d?.useMaxWidth));
  }, 'draw'),
  Te = { draw: De },
  $e = o((e) => '', 'getStyles'),
  Ce = $e,
  _e = { parser: Ee, db: F, renderer: Te, styles: Ce };
