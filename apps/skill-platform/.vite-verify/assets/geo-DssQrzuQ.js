...
import { aO as k } from './index-C2avURFS.js';
...
`,
  A = () => {
    ...
    return ((t.textContent = P), t);
  },
  d = (t) => document.createElementNS(M, t),
  w = (t) => {
    const e = t;
    if (e?.type === 'FeatureCollection' && Array.isArray(e.features)) return e;
    if (e?.type === 'Feature') return { type: 'FeatureCollection', features: [e] };
  ...
  $ = (t) => {
    const e = new DOMParser().parseFromString(t, 'application/xml'),
      r = e.querySelector('parsererror');
    if (r) throw new Error(r.textContent || 'XML 解析失败');
    return e;
  },
  L = (t) => {
    const e = (t || 'geojson').trim().toLowerCase();
    return e === 'json' ? 'geojson' : e === 'shapefile' ? 'shp' : e;
  },
  O = async (t, e) => {
    if (e === 'geojson') return w(JSON.parse(await k(t)));
    if (e === 'kml' || e === 'gpx') {
      const r = await E(() => import('./togeojson.es-7LcLrbIW.js'), [], import.meta.url),
        o = $(await k(t));
      return w(e === 'kml' ? r.kml(o) : r.gpx(o));
    }
    if (e === 'shp') {
      const { default: r } = await E(
        async () => {
          const { default: o } = await import('./index-Dj46tQV6.js');
          return { default: o };
        },
        [],
        import.meta.url,
  ...
  F = (t, e) => {
    if (t)
      switch (t.type) {
        ...
        case 'Polygon':
          t.coordinates.forEach((r) => r.forEach(e));
          break;
        case 'MultiPolygon':
          t.coordinates.forEach((r) => r.forEach((o) => o.forEach(e)));
          break;
        case 'GeometryCollection':
          t.geometries.forEach((r) => F(r, e));
          break;
      }
  },
  z = (t) => {
    ...
    return (
      ...
      !Number.isFinite(e) || !Number.isFinite(r) ? null : { minX: e, minY: r, maxX: o, maxY: n }
    );
  },
  _ = (t, e) => {
    if (!e) return [g / 2, l / 2];
    const r = Math.max(1e-9, e.maxX - e.minX),
      o = Math.max(1e-9, e.maxY - e.minY),
      n = Math.min((g - C * 2) / r, (l - C * 2) / o),
      a = (g - r * n) / 2,
      i = (l - o * n) / 2,
      c = a + (t[0] - e.minX) * n,
      s = l - (i + (t[1] - e.minY) * n);
    return [Number(c.toFixed(2)), Number(s.toFixed(2))];
  },
  S = (t, e, r = !1) => {
    const o = t.filter((c) => Number.isFinite(c[0]) && Number.isFinite(c[1])).map((c) => _(c, e));
    if (!o.length) return '';
    const [n, ...a] = o,
      i = [`M${n[0]} ${n[1]}`, ...a.map((c) => `L${c[0]} ${c[1]}`)];
    return (r && i.push('Z'), i.join(' '));
  },
  p = (t, e, r) => {
    if (e)
      switch (e.type) {
        case 'Point': {
          const [o, n] = _(e.coordinates, r),
            a = d('circle');
          (a.setAttribute('cx', String(o)),
            a.setAttribute('cy', String(n)),
            a.setAttribute('r', '4'),
            t.appendChild(a));
          break;
        }
        case 'MultiPoint':
          e.coordinates.forEach((o) => p(t, { type: 'Point', coordinates: o }, r));
          break;
        case 'LineString': {
          const o = d('path');
          (o.setAttribute('d', S(e.coordinates, r)), t.appendChild(o));
          break;
        }
        case 'MultiLineString':
          e.coordinates.forEach((o) => p(t, { type: 'LineString', coordinates: o }, r));
          break;
        case 'Polygon':
          e.coordinates.forEach((o) => {
            const n = d('path');
            (n.classList.add('geo-polygon'), n.setAttribute('d', S(o, r, !0)), t.appendChild(n));
          });
          break;
        case 'MultiPolygon':
          e.coordinates.forEach((o) => {
            p(t, { type: 'Polygon', coordinates: o }, r);
          });
          break;
        case 'GeometryCollection':
          e.geometries.forEach((o) => p(t, o, r));
          break;
      }
  },
  I = (t) => {
    ...
    return (
      t.forEach((r) => {
        var o;
        const n = ((o = r.geometry) === null || o === void 0 ? void 0 : o.type) || 'Null';
        e.set(n, (e.get(n) || 0) + 1);
      }),
      [...e.entries()]
    );
  ...
  X = (t) =>
    t
      ? `${t.minX.toFixed(5)}, ${t.minY.toFixed(5)} -> ${t.maxX.toFixed(5)}, ${t.maxY.toFixed(5)}`
      : '-',
  N = (t, e, r) => {
    ...
    ((a.textContent = String(r)), o.append(n, a), t.appendChild(o));
  },
  j = (t, e) => {
    const r = document.createElement('div');
    r.className = 'geo-viewer';
    const o = t.features || [],
      n = z(o),
      a = document.createElement('aside');
    a.className = 'geo-panel';
    const i = document.createElement('span');
    i.textContent = e.toUpperCase();
    const c = document.createElement('h2');
    c.textContent = '地理数据预览';
    const s = document.createElement('dl');
    (N(s, '要素数', o.length), N(s, '范围', X(n)));
    const u = document.createElement('div');
    u.className = 'geo-counts';
    const b = document.createElement('strong');
    ((b.textContent = '几何类型'),
      u.appendChild(b),
      I(o).forEach(([x, G]) => {
        const y = document.createElement('p');
        ((y.textContent = `${x}: ${G}`), u.appendChild(y));
      }),
      a.append(i, c, s, u));
    const h = document.createElement('main');
    h.className = 'geo-map';
    const m = d('svg');
    (m.setAttribute('viewBox', `0 0 ${g} ${l}`),
      m.setAttribute('role', 'img'),
      m.setAttribute('aria-label', '地理数据 SVG 预览'));
    const f = d('rect');
    (f.setAttribute('width', String(g)),
      f.setAttribute('height', String(l)),
      f.setAttribute('rx', '8'));
    const v = d('g');
    return (
      o.forEach((x) => p(v, x.geometry, n)),
      m.append(f, v),
      h.appendChild(m),
      r.append(a, h),
      r
...
async function J(t, e, r) {
  ...
  ((a.className = 'geo-state'), (a.textContent = '正在解析地理数据...'), e.replaceChildren(n, a));
  try {
    const i = await O(t, o);
    e.replaceChildren(n, j(i, o));
  } catch (i) {
    (console.error(i),
      a.classList.add('error'),
      (a.textContent = i instanceof Error ? i.message : String(i)));
  }
  return {
    $el: e,
    unmount() {
