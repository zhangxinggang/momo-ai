...
import { I as he } from './chunk-QZHKN3VN-BdOwWt8C.js';
import { s as Ee } from './markdown-it-vendor-DL4wSELR.js';
...
var m = { NORMAL: 0, REVERSE: 1, HIGHLIGHT: 2, MERGE: 3, CHERRY_PICK: 4 },
  Be = ke.gitGraph,
  S = h(() => we({ ...Be, ...J().gitGraph }), 'getConfig'),
  d = new he(() => {
    const e = S(),
      r = e.mainBranchName,
      t = e.mainBranchOrder;
...
var Te = h(function (e) {
  ...
  Re = h(function (e) {
    ...
    const i = S();
    ((t = B.sanitizeText(t, i)),
      (r = B.sanitizeText(r, i)),
      (o = o?.map((a) => B.sanitizeText(a, i))));
    const n = {
    ...
    ((d.records.head = n),
      ...
      d.records.commits.has(n.id) && w.warn(`Commit ID ${n.id} already exists`),
      d.records.commits.set(n.id, n),
      d.records.branches.set(d.records.currBranch, n.id),
      w.debug('in pushCommit ' + n.id));
  }, 'commit'),
  Ie = h(function (e) {
    let r = e.name;
    const t = e.order;
    ...
    (d.records.branches.set(r, d.records.head != null ? d.records.head.id : null),
      d.records.branchConfig.set(r, { name: r, order: t }),
      ae(r),
  ...
  Oe = h((e) => {
    ...
    const n = d.records.branches.get(d.records.currBranch),
      a = d.records.branches.get(r),
      l = n ? d.records.commits.get(n) : void 0,
      f = a ? d.records.commits.get(a) : void 0;
    if (l && f && l.branch === r) throw new Error(`Cannot merge branch '${r}' into itself.`);
    ...
    ((d.records.head = $),
      d.records.commits.set($.id, $),
      d.records.branches.set(d.records.currBranch, $.id),
  ...
  _e = h(function (e) {
    ...
    if (
      ((r = B.sanitizeText(r, i)),
      (t = B.sanitizeText(t, i)),
      (s = s?.map((l) => B.sanitizeText(l, i))),
      (o = B.sanitizeText(o, i)),
      !r || !d.records.commits.has(r))
    ) {
      const l = new Error(
    ...
    if (!t || !d.records.commits.has(t)) {
      ...
      ((d.records.head = g),
        d.records.commits.set(g.id, g),
        d.records.branches.set(d.records.currBranch, g.id),
        w.debug(d.records.branches),
  ...
  ae = h(function (e) {
    if (((e = B.sanitizeText(e, S())), d.records.branches.has(e))) {
      ...
      r === void 0 || !r
        ? (d.records.head = null)
        : (d.records.head = d.records.commits.get(r) ?? null);
    } else {
      const r = new Error(
...
var Ge = h(function () {
  ...
  Ne = h((e, r) => {
    ...
    for (const t of e.statements) Fe(t, r);
  }, 'populate'),
  Fe = h((e, r) => {
    const s = {
      Commit: h((o) => r.commit(ze(o)), 'Commit'),
      Branch: h((o) => r.branch(Ye(o)), 'Branch'),
      Merge: h((o) => r.merge(je(o)), 'Merge'),
      Checkout: h((o) => r.checkout(Ue(o)), 'Checkout'),
      CherryPicking: h((o) => r.cherryPick(Ke(o)), 'CherryPicking'),
    }[e.$type];
  ...
  Ye = h((e) => ({ name: e.name, order: e.order ?? 0 }), 'parseBranch'),
  je = h(
    (e) => ({
      branch: e.branch,
      id: e.id ?? '',
  ...
  R = 2,
  G = 8,
  j = new Set(['redux', 'redux-dark', 'redux-color', 'redux-dark-color']),
  X = 12,
  Z = new Set(['redux-color', 'redux-dark-color']),
  Xe = new Set(['dark', 'redux-dark', 'redux-dark-color', 'neo-dark']),
  H = h((e, r, t = !1) => (t && e > 0 ? ((e - 1) % (r - 1)) + 1 : e % r), 'calcColorIndex'),
  C = new Map(),
  E = new Map(),
  F = 30,
  P = new Map(),
  ...
  ce = h((e) => {
    ...
    return (
      y === 'BT'
        ? ((t = h((o, i) => o <= i, 'comparisonFunc')), (s = 1 / 0))
        : ((t = h((o, i) => o >= i, 'comparisonFunc')), (s = 0)),
      e.forEach((o) => {
  ...
  rr = h((e) => er(e) + _, 'calculateCommitPosition'),
  tr = h((e, r) => {
  ...
  nr = h((e, r, t, s, o, i) => {
    ...
    if (i === m.HIGHLIGHT)
      (e
        ...
        .attr('width', a ? 14 : 20)
        .attr('height', a ? 14 : 20)
        .attr('class', `commit ${r.id} commit-highlight${H(o, G, l)} ${s}-outer`),
        e
          .append('rect')
          .attr('x', t.x - 6 + (a ? 2 : 0))
          .attr('y', t.y - 6 + (a ? 2 : 0))
          .attr('width', a ? 8 : 12)
          .attr('height', a ? 8 : 12)
          .attr('class', `commit ${r.id} commit${H(o, G, l)} ${s}-inner`));
      ...
      if (
        ...
        g.attr('cy', t.y),
        g.attr('r', a ? 7 : 10),
        g.attr('class', `commit ${r.id} commit${H(o, G, l)}`),
        i === m.MERGE)
      ) {
        ...
        ($.attr('cx', t.x),
          $.attr('cy', t.y),
          $.attr('r', a ? 5 : 6),
          $.attr('class', `commit ${s} ${r.id} commit${H(o, G, l)}`));
      }
      if (i === m.REVERSE) {
        const $ = e.append('path'),
          c = a ? 4 : 5;
  ...
  sr = h((e, r, t, s, o) => {
    if (
      r.type !== m.CHERRY_PICK &&
      ((r.customId && r.type === m.MERGE) || r.type !== m.MERGE) &&
      o.showCommitLabel
    ) {
      const i = e.append('g'),
        n = i.insert('rect').attr('class', 'commit-label-bkg'),
        a = i
          .append('text')
          .attr('x', s)
          .attr('y', t.y + 25)
      ...
      if (
        l &&
        (n
          .attr('x', t.posWithOffset - l.width / 2 - R)
          .attr('y', t.y + 13.5)
          .attr('width', l.width + 2 * R)
          .attr('height', l.height + 2 * R),
        y === 'TB' || y === 'BT'
          ? (n.attr('x', t.x - (l.width + 4 * L + 5)).attr('y', t.y - 12),
            a.attr('x', t.x - (l.width + 4 * L)).attr('y', t.y + l.height - 12))
          : a.attr('x', t.posWithOffset - l.width / 2),
        o.rotateCommitLabel)
      )
        if (y === 'TB' || y === 'BT')
          (a.attr('transform', 'rotate(-45, ' + t.x + ', ' + t.y + ')'),
            n.attr('transform', 'rotate(-45, ' + t.x + ', ' + t.y + ')'));
        else {
          const f = -7.5 - ((l.width + 10) / 25) * 9.5,
            g = 10 + (l.width / 25) * 8.5;
          i.attr(
            'transform',
    ...
    if (r.tags.length > 0) {
      ...
      for (const l of r.tags.reverse()) {
        ...
        ((i = Math.max(i, c.width)),
          (n = Math.max(n, c.height)),
          $.attr('x', t.posWithOffset - c.width / 2),
          a.push({ tag: $, hole: g, rect: f, yOffset: o }),
          (o += 20));
      }
      ...
      ${t.posWithOffset - i / 2 - L},${x + c + R}`,
        ...
        ${t.x + O},${u + c + 2}`,
            ...
            .attr('transform', 'translate(12,12) rotate(45, ' + t.x + ',' + s + ')'),
            f
              .attr('cx', t.x + L / 2)
              .attr('cy', u)
              .attr('transform', 'translate(12,12) rotate(45, ' + t.x + ',' + s + ')'),
            l
              .attr('x', t.x + 5)
              .attr('y', u + 3)
              .attr('transform', 'translate(14,14) rotate(45, ' + t.x + ',' + s + ')'));
        }
      }
    }
  }, 'drawCommitTags'),
  ...
  ir = h((e, r, t, s) => {
    ...
    } else return r === 'TB' ? F : r === 'BT' ? (s.get(e.id) ?? o).y - _ : 0;
    return 0;
  }, 'calculatePosition'),
  dr = h((e, r, t) => {
    const s = y === 'BT' && t ? r : r + O,
      o = C.get(e.branch)?.pos,
      i = y === 'TB' || y === 'BT' ? C.get(e.branch)?.pos : s;
    if (i === void 0 || o === void 0) throw new Error(`Position were undefined for commit ${e.id}`);
    const n = j.has(q().theme ?? ''),
      a = y === 'TB' || y === 'BT' ? s : o + (n ? X / 2 + 1 : -2);
    return { x: i, y: a, posWithOffset: s };
  }, 'getCommitPosition'),
  re = h((e, r, t, s) => {
    const o = e.append('g').attr('class', 'commit-bullets'),
      i = e.append('g').attr('class', 'commit-labels');
    let n = y === 'TB' || y === 'BT' ? F : 0;
    const a = [...r.keys()],
      l = s.parallelCommits ?? !1,
      f = h(($, c) => {
        const x = r.get($)?.seq,
          u = r.get(c)?.seq;
    ...
    let g = a.sort(f);
    (y === 'BT' && (l && Ze(g, r, n), (g = g.reverse())),
      g.forEach(($) => {
        const c = r.get($);
        if (!c) throw new Error(`Commit not found for key ${$}`);
        l && (n = ir(c, y, n, E));
        const x = dr(c, n, l);
        if (t) {
          const u = cr(c),
            p = c.customType ?? c.type,
            b = C.get(c.branch)?.index ?? 0;
          (nr(o, c, x, u, b, p), sr(i, c, x, n, s), or(i, c, x, n));
        }
        (y === 'TB' || y === 'BT'
          ? E.set(c.id, { x: x.x, y: x.posWithOffset })
          : E.set(c.id, { x: x.posWithOffset, y: x.y }),
          (n = y === 'BT' && l ? n + _ : n + _ + O),
          n > I && (I = n));
      }));
  }, 'drawCommits'),
  lr = h((e, r, t, s, o) => {
    const n = (y === 'TB' || y === 'BT' ? t.x < s.x : t.y < s.y) ? r.branch : e.branch,
      a = h((f) => f.branch === n, 'isOnBranchToGetCurve'),
      l = h((f) => f.seq > e.seq && f.seq < r.seq, 'isBetweenCommits');
    return [...o.values()].some((f) => l(f) && a(f));
  }, 'shouldRerouteArrow'),
  W = h((e, r, t = 0) => {
    const s = e + Math.abs(e - r) / 2;
    if (t > 5) return s;
    if (z.every((n) => Math.abs(n - s) >= 10)) return (z.push(s), s);
  ...
  hr = h((e, r, t, s) => {
    ...
    let f = '',
      ...
      $ = 0,
      c = 0,
      x = C.get(t.branch)?.index;
    t.type === m.MERGE && r.id !== t.parents[0] && (x = C.get(r.branch)?.index);
    let u;
    if (l) {
      ((f = 'A 10 10, 0, 0, 0,'), (g = 'A 10 10, 0, 0, 1,'), ($ = 10), (c = 10));
      const p = n.y < a.y ? W(n.y, a.y) : W(a.y, n.y),
        b = n.x < a.x ? W(n.x, a.x) : W(a.x, n.x);
      y === 'TB'
        ? n.x < a.x
          ? (u = `M ${n.x} ${n.y} L ${b - $} ${n.y} ${g} ${b} ${n.y + c} L ${b} ${a.y - $} ${f} ${b + c} ${a.y} L ${a.x} ${a.y}`)
          : ((x = C.get(r.branch)?.index),
            (u = `M ${n.x} ${n.y} L ${b + $} ${n.y} ${f} ${b} ${n.y + c} L ${b} ${a.y - $} ${g} ${b - c} ${a.y} L ${a.x} ${a.y}`))
        : y === 'BT'
          ? n.x < a.x
            ? (u = `M ${n.x} ${n.y} L ${b - $} ${n.y} ${f} ${b} ${n.y - c} L ${b} ${a.y + $} ${g} ${b + c} ${a.y} L ${a.x} ${a.y}`)
            : ((x = C.get(r.branch)?.index),
              (u = `M ${n.x} ${n.y} L ${b + $} ${n.y} ${g} ${b} ${n.y - c} L ${b} ${a.y + $} ${f} ${b - c} ${a.y} L ${a.x} ${a.y}`))
          : n.y < a.y
            ? (u = `M ${n.x} ${n.y} L ${n.x} ${p - $} ${f} ${n.x + c} ${p} L ${a.x - $} ${p} ${g} ${a.x} ${p + c} L ${a.x} ${a.y}`)
            : ((x = C.get(r.branch)?.index),
              (u = `M ${n.x} ${n.y} L ${n.x} ${p + $} ${g} ${n.x + c} ${p} L ${a.x - $} ${p} ${f} ${a.x} ${p - c} L ${a.x} ${a.y}`));
    } else
      ((f = 'A 20 20, 0, 0, 0,'),
        (g = 'A 20 20, 0, 0, 1,'),
        ($ = 20),
        ...
        y === 'TB'
          ? (n.x < a.x &&
              (t.type === m.MERGE && r.id !== t.parents[0]
                ? (u = `M ${n.x} ${n.y} L ${n.x} ${a.y - $} ${f} ${n.x + c} ${a.y} L ${a.x} ${a.y}`)
                : (u = `M ${n.x} ${n.y} L ${a.x - $} ${n.y} ${g} ${a.x} ${n.y + c} L ${a.x} ${a.y}`)),
            n.x > a.x &&
              ((f = 'A 20 20, 0, 0, 0,'),
              (g = 'A 20 20, 0, 0, 1,'),
              ($ = 20),
              (c = 20),
              t.type === m.MERGE && r.id !== t.parents[0]
                ? (u = `M ${n.x} ${n.y} L ${n.x} ${a.y - $} ${g} ${n.x - c} ${a.y} L ${a.x} ${a.y}`)
                : (u = `M ${n.x} ${n.y} L ${a.x + $} ${n.y} ${f} ${a.x} ${n.y + c} L ${a.x} ${a.y}`)),
    ...
    e.append('path')
      .attr('d', u)
      .attr('class', 'arrow arrow' + H(x, G, i));
  }, 'drawArrow'),
  $r = h((e, r) => {
  ...
  fr = h((e, r, t, s) => {
    const { look: o, theme: i, themeVariables: n } = q(),
    ...
    r.forEach((c, x) => {
      const u = H(x, f ? l : G, g),
        p = C.get(c.name)?.pos;
      if (p === void 0) throw new Error(`Position not found for branch ${c.name}`);
      const b = y === 'TB' || y === 'BT' ? p : f ? p + X / 2 + 1 : p - 2,
        k = $.append('line');
      (k.attr('x1', 0),
        k.attr('y1', b),
        k.attr('x2', I),
        k.attr('y2', b),
        k.attr('class', 'branch branch' + u),
        y === 'TB'
          ? (k.attr('y1', F), k.attr('x1', p), k.attr('y2', I), k.attr('x2', p))
          : y === 'BT' && (k.attr('y1', I), k.attr('x1', p), k.attr('y2', F), k.attr('x2', p)),
        z.push(b));
      const U = c.name,
        D = oe(U),
        T = $.insert('rect'),
      ...
      const v = D.getBBox(),
        ...
        N = f ? 16 : 0,
        A = f ? X : 0;
      (o === 'neo' && T.attr('data-look', 'neo'),
        T.attr('class', 'branchLabelBkg label' + u)
          .attr('style', o === 'neo' ? `filter:${f ? `url(#${s}-drop-shadow)` : a}` : '')
          .attr('rx', ee)
          .attr('ry', ee)
          .attr('x', -v.width - 4 - (t.rotateCommitLabel === !0 ? 30 : 0))
          .attr('y', -v.height / 2 + 10)
          .attr('width', v.width + 18 + N)
          .attr('height', v.height + 4 + A),
        M.attr(
          'transform',
          'translate(' +
            (-v.width - 14 - (t.rotateCommitLabel === !0 ? 30 : 0) + N / 2) +
            ', ' +
            (b - v.height / 2 - 2) +
            ')',
        ),
        y === 'TB'
          ? (T.attr('x', p - v.width / 2 - 10).attr('y', 0),
            M.attr('transform', 'translate(' + (p - v.width / 2 - 5) + ', 0)'),
            f &&
              (T.attr('transform', `translate(${-N / 2 - 3}, ${-A - 10})`),
              M.attr(
                'transform',
                'translate(' + (p - v.width / 2 - 5) + ', ' + (-A * 2 + 7) + ')',
              )))
          : y === 'BT'
            ? (T.attr('x', p - v.width / 2 - 10).attr('y', I),
              M.attr('transform', 'translate(' + (p - v.width / 2 - 5) + ', ' + I + ')'),
              f &&
                (T.attr('transform', `translate(${-N / 2 - 3}, ${A + 10})`),
                M.attr(
                  'transform',
  ...
  gr = h(function (e, r, t, s, o) {
    return (
      C.set(e, { pos: r, index: t }),
      (r += 50 + (o ? 40 : 0) + (y === 'TB' || y === 'BT' ? s.width / 2 : 0)),
      r
    );
  }, 'setBranchPosition'),
  ur = h(function (e, r, t, s) {
    (Je(),
      w.debug(
...
`,
    ...
    const i = o.getConfig(),
      n = i.rotateCommitLabel ?? !1;
    P = o.getCommits();
    const a = o.getBranchesAsObjArray();
    y = o.getDirection();
    const l = Ee(`[id="${r}"]`),
      { look: f, theme: g, themeVariables: $ } = q(),
      { useGradient: c, gradientStart: x, gradientStop: u, filterColor: p } = $;
    if (c) {
      const k = l
        .append('defs')
        ...
        .attr('x2', '100%')
        .attr('y2', '0%');
      (k.append('stop').attr('offset', '0%').attr('stop-color', x).attr('stop-opacity', 1),
        k.append('stop').attr('offset', '100%').attr('stop-color', u).attr('stop-opacity', 1));
    }
    f === 'neo' &&
      j.has(g ?? '') &&
      l
    ...
    (a.forEach((k, U) => {
      const D = oe(k.name),
        T = l.append('g'),
        K = T.insert('g').attr('class', 'branchLabel'),
        M = K.insert('g').attr('class', 'label branch-label');
      M.node()?.appendChild(D);
      const v = D.getBBox();
      ((b = gr(k.name, b, U, v, n)), M.remove(), K.remove(), T.remove());
    }),
      re(l, P, !1, i),
      i.showBranches && fr(l, a, i, r),
      $r(l, P),
      re(l, P, !0, i),
      me.insertTitle(l, 'gitTitleText', i.titleTopMargin ?? 0, o.getDiagramTitle()),
      pe(void 0, l, i.diagramPadding, i.useMaxWidth));
  }, 'draw'),
  yr = { draw: ur },
  ie = 8,
  de = new Set(['redux', 'redux-dark', 'redux-color', 'redux-dark-color']),
  xr = new Set(['redux-color', 'redux-dark-color']),
  mr = new Set(['neo', 'neo-dark']),
  pr = new Set(['dark', 'redux-dark', 'redux-dark-color', 'neo-dark']),
  br = new Set(['redux', 'redux-dark', 'redux-color', 'redux-dark-color', 'neo', 'neo-dark']),
  wr = h((e) => {
    const { svgId: r } = e;
    let t = '';
    if (e.useGradient && r)
  ...
  kr = h((e) => {
    const r = J(),
      { theme: t, themeVariables: s } = r,
      { borderColorArray: o } = s,
      i = de.has(t);
    if (mr.has(t)) {
      let n = '';
...
`;
  ...
  Er = Cr,
  _r = { parser: Ve, db: se, renderer: yr, styles: Er };
export { _r as diagram };
