...
import { aQ as f, b5 as v, ad as w } from './index-C2avURFS.js';
...
`,
  ...
  L = async (i) =>
    await new Promise((e, o) => {
      ...
      ((r.onload = (t) => {
        var a;
        const m = (a = t.target) === null || a === void 0 ? void 0 : a.result;
        if (typeof m == 'string') {
          e(m);
          return;
        }
        o(new Error('Unable to read image data URL.'));
      }),
        (r.onerror = () => o(r.error || new Error('Unable to read image data URL.'))),
        r.readAsDataURL(i));
    }),
  Z = async (i, e) => {
    ...
    return L(new Blob([i], { type: z(o) }));
  },
  C = (i) => Math.min(5, Math.max(0.1, Number(i.toFixed(2)))),
  h = (i, e, o) => {
    if (e > 0) {
      i.style.height = `${Math.max(1, Math.round(e * o))}px`;
      return;
    }
    i.style.height = `${o * 100}%`;
  },
  I = (i) => {
    ...
    return (
      ...
      e.append(o, r),
      {
        ...
        destroy() {
          (r.removeEventListener('click', t), o.removeEventListener('click', t), e.remove());
        },
      }
    );
  };
async function R(i, e, o) {
  const r = await Z(i, o);
  let t = 1,
    a = 0;
  const m = w(),
    n = document.createElement('div');
  ((n.className = 'image-viewer'), (n.dataset.viewerZoomProvider = 'image'));
  const g = document.createElement('div');
  g.className = 'image-stage';
  const s = document.createElement('img');
  ((s.alt = '图片'), (s.src = r), g.append(s), n.append(g));
  const d = I(r),
    l = () => d.open();
  (s.addEventListener('click', l), document.body.append(d.element));
  const p = () => {
      ((a = n.clientHeight || 0), h(s, a, t), m.emit());
    },
  ...
  return (
    ...
    e.replaceChildren(k(), n),
    p(),
    {
      $el: e,
      unmount() {
        (v(n), b.disconnect(), s.removeEventListener('click', l), d.destroy(), e.replaceChildren());
      },
    }
  );
}
