...
function X(e) {
  const t = e.regex,
    ...
    s = { $pattern: p, keyword: G, literal: K, built_in: J, 'variable.language': H },
    _ = '[0-9](_?[0-9])*',
    E = `\\.(${_})`,
    ...
    c = { className: 'subst', begin: '\\$\\{', end: '\\}', keywords: s, contains: [] },
    m = {
      begin: '.?html`',
      end: '',
    ...
    R = { className: 'string', begin: '`', end: '`', contains: [e.BACKSLASH_ESCAPE, c] },
    g = {
      className: 'comment',
      variants: [
        e.COMMENT(/\/\*\*(?!\/)/, '\\*/', {
    ...
    y = [e.APOS_STRING_MODE, e.QUOTE_STRING_MODE, m, T, f, R, { match: /\$\d+/ }, S];
  c.contains = y.concat({ begin: /\{/, end: /\}/, keywords: s, contains: ['self'].concat(y) });
  const I = [].concat(g, c.contains),
    b = I.concat([{ begin: /(\s*)\(/, end: /\)/, keywords: s, contains: ['self'].concat(I) }]),
    i = {
      className: 'params',
      begin: /(\s*)\(/,
      end: /\)/,
  ...
  return {
    ...
    contains: [
      ...
      S,
      O,
      { scope: 'attr', match: n + t.lookahead(':'), relevance: 0 },
      z,
      {
      ...
      { beginKeywords: 'while if switch catch for' },
      {
        ...
        returnBegin: !0,
        label: 'func.def',
        contains: [i, e.inherit(e.TITLE_MODE, { begin: n, className: 'title.function' })],
      },
      { match: /\.\.\./, relevance: 0 },
      Z,
      { match: '\\$' + n, relevance: 0 },
      { match: [/\bconstructor(?=\s*\()/], className: { 1: 'title.function' }, contains: [i] },
      $,
      k,
      x,
      F,
