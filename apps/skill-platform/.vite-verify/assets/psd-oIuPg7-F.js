...
import { ad as H, aQ as j, b5 as q } from './index-C2avURFS.js';
import './markdown-it-vendor-DL4wSELR.js';
import { _ as O } from './markdown-vendor-DldLOD9R.js';
...
`,
  d = (r, n, e, i) => {
    const a = r.createElement(n);
    return (e && (a.className = e), typeof i == 'string' && (a.textContent = i), a);
  },
  B = (r) => {
    const n = r.createElement('style');
    return ((n.textContent = T), n);
  },
  V = (r, n) => {
    if (!n) return;
    const e = r.defaultView || (typeof window < 'u' ? window : void 0);
    if (e && n instanceof e.HTMLCanvasElement) return n;
    if (e && n instanceof e.ImageData) {
      const i = r.createElement('canvas');
      return (
        (i.width = n.width),
        (i.height = n.height),
        i.getContext('2d')?.putImageData(n, 0, 0),
        i
      );
    }
    if (typeof n == 'object' && 'data' in n && 'width' in n && 'height' in n) {
      const i = n,
        a = r.createElement('canvas');
      ((a.width = i.width), (a.height = i.height));
      const p = new Uint8ClampedArray(i.data.length);
      p.set(i.data);
      const c = e?.ImageData || ImageData;
      return (a.getContext('2d')?.putImageData(new c(p, i.width, i.height), 0, 0), a);
    }
  },
  F = (r, n, e = 0, i = '') =>
    Array.isArray(n)
      ? n.flatMap((a, p) => {
          const c = String(a.name || `Layer ${p + 1}`),
            h = `${i}${p}`,
            l = V(r, a.canvas || a.imageData);
          return [
            {
              id: h,
              name: c,
              ...
              top: Number(a.top || 0),
              width: Math.max(0, Number(a.right || 0) - Number(a.left || 0)),
              height: Math.max(0, Number(a.bottom || 0) - Number(a.top || 0)),
              hidden: !!a.hidden,
              canvas: l,
            },
            ...F(r, a.children, e + 1, `${h}.`),
          ];
        })
      : [],
  Q = (r) => Math.min(4, Math.max(0.1, Number(r.toFixed(2))));
async function K(r, n) {
  const e = n.ownerDocument || document,
    { readPsd: i } = await O(
      async () => {
        const { readPsd: t } = await import('./index-C6jCrrYJ.js');
        return { readPsd: t };
      },
      __vite__mapDeps([0, 1, 2, 3, 4, 5, 6, 7]),
      import.meta.url,
    ),
    a = i(r, { useImageData: !0 }),
    p = V(e, a.canvas || a.imageData),
    c = F(e, a.children),
    h = c.filter((t) => t.canvas),
    l = new Set(h.filter((t) => !t.hidden).map((t) => t.id));
  let s = 1;
  const z = H(),
    b = d(e, 'div', 'psd-viewer');
  b.dataset.viewerZoomProvider = 'psd';
  const $ = d(e, 'div', 'psd-toolbar'),
    L = d(e, 'div', 'psd-title');
  L.append(
    d(e, 'strong', void 0, 'PSD 图层预览'),
    d(e, 'span', void 0, `${a.width || 0} x ${a.height || 0} · ${c.length} layers · ag-psd`),
  );
  const D = d(e, 'div', 'psd-actions'),
    v = d(e, 'button', void 0, '-'),
    I = d(e, 'span', void 0, '100%'),
    w = d(e, 'button', void 0, '+'),
    k = d(e, 'button', void 0, '适合'),
    y = d(e, 'button', void 0, '全显'),
    E = d(e, 'button', void 0, '全隐');
  ([v, w, k, y, E].forEach((t) => {
    t.type = 'button';
  }),
    D.append(v, I, w, k, y, E),
    $.append(L, D));
  const S = d(e, 'div', 'psd-layout'),
    M = d(e, 'aside', 'psd-sidebar'),
    Z = d(e, 'div', 'psd-sidebar-header');
  Z.append(d(e, 'span', void 0, '图层'), d(e, 'span', void 0, `${h.length} 可重绘`));
  const u = d(e, 'ul', 'psd-layer-list');
  M.append(Z, u);
  const A = d(e, 'main', 'psd-stage'),
    C = d(e, 'div', 'psd-canvas-wrap'),
    N = d(e, 'div', 'psd-canvas-shell'),
    g = d(e, 'canvas');
  ((g.width = Math.max(1, Number(a.width || p?.width || 1))),
    (g.height = Math.max(1, Number(a.height || p?.height || 1))),
    N.appendChild(g),
    C.appendChild(N),
    A.appendChild(C),
    S.append(M, A),
    b.append($, S),
    n.replaceChildren(B(e), b));
  const x = () => {
    const t = g.getContext('2d');
    if (!t) return;
    t.clearRect(0, 0, g.width, g.height);
    const f = h.filter((o) => l.has(o.id));
    if (!f.length && p) {
      t.drawImage(p, 0, 0);
      return;
    }
    f.slice()
      .reverse()
      .forEach((o) => {
        o.canvas && t.drawImage(o.canvas, o.left, o.top);
      });
  };
  (c.length || u.appendChild(d(e, 'li', 'psd-empty', '未发现可展示图层，使用合成图预览。')),
    c.forEach((t) => {
      const f = d(e, 'li', 'psd-layer');
      f.style.paddingLeft = `${8 + t.depth * 14}px`;
      const o = d(e, 'input');
      ((o.type = 'checkbox'),
        (o.disabled = !t.canvas),
        (o.checked = l.has(t.id)),
        o.addEventListener('change', () => {
          (o.checked ? l.add(t.id) : l.delete(t.id), x());
        }));
      const _ = d(e, 'div');
      (_.append(
        d(e, 'strong', void 0, t.name),
        d(
          e,
          'span',
          void 0,
          `${t.left},${t.top} · ${t.width} x ${t.height}${t.hidden ? ' · hidden' : ''}`,
        ),
      ),
        f.append(o, _),
        u.appendChild(f));
  ...
  const P = () => ({
    ...
    m = (t) => (
      ...
      (I.textContent = `${Math.round(s * 100)}%`),
      z.emit(),
      P()
    );
  return (
    v.addEventListener('click', () => m(s - 0.1)),
    w.addEventListener('click', () => m(s + 0.1)),
    k.addEventListener('click', () => m(1)),
    y.addEventListener('click', () => {
      (h.forEach((t) => l.add(t.id)),
        u.querySelectorAll('input[type="checkbox"]').forEach((t) => {
