...
var u = {
    ...
    lineEditor: { edit: 'Редактирование строки', exit: 'Выход из редактора строки' },
    elementLock: {
      lock: 'Блокировать',
  ...
  E = {
    ...
    hint_emptyLibrary:
      'Выберите объект на холсте, чтобы добавить его сюда, или установите библиотеку из публичного репозитория ниже.',
  ...
  e = {
    ...
    couldNotCreateShareableLinkTooBig:
      'Нельзя создать ссылку, чтобы поделиться. Сцена слишком большая',
    ...
    uploadedSecurly:
      'Загружаемые данные защищена сквозным шифрованием, что означает, что сервер Excalidraw и третьи стороны не могут прочитать содержимое.',
    loadSceneOverridePrompt:
      'Загрузка рисунка приведёт к замене имеющегося содержимого. Вы хотите продолжить?',
...
(Если вы хотите оставить ваш локальный рисунок, просто закройте вкладку браузера)`,
    ...
    imageDoesNotContainScene:
      'Это изображение не содержит данных сцены. Вы включили встраивание сцены во время экспорта?',
    cannotRestoreFromImage: 'Сцена не может быть восстановлена из этого изображения',
    invalidSceneUrl:
      'Невозможно импортировать сцену с предоставленного URL. Неверный формат, или не содержит верных Excalidraw JSON данных.',
    resetLibrary: 'Это очистит вашу библиотеку. Вы уверены?',
    removeItemsFromsLibrary: 'Удалить {{count}} объект(ов) из библиотеки?',
    invalidEncryptionKey:
      'Ключ шифрования должен состоять из 22 символов. Одновременное редактирование отключено.',
...
Ваши изменения не будут сохранены!`,
  ...
  B = {
    ...
    svgImageInsertError:
      'Не удалось вставить изображение SVG. Разметка SVG выглядит недействительной.',
    failedToFetchImage: 'Не удалось получить изображение.',
    invalidSVGString: 'Некорректный SVG.',
    cannotResolveCollabServer:
      'Не удалось подключиться к серверу совместного редактирования. Перезагрузите страницу и повторите попытку.',
    importLibraryError: 'Не удалось загрузить библиотеку',
    collabSaveFailed:
      'Не удалось сохранить в базу данных. Если проблема повторится, нужно будет сохранить файл локально, чтобы быть уверенным, что вы не потеряете вашу работу.',
    collabSaveFailed_sizeExceeded:
      'Не удалось сохранить в базу данных. Похоже, что холст слишком большой. Нужно сохранить файл локально, чтобы быть уверенным, что вы не потеряете вашу работу.',
    imageToolNotSupported: 'Изображения отключены.',
    brave_measure_text_error: {
      line1:
        'Похоже, вы используете браузер Brave с включенной опцией <bold>Агрессивно блокировать отслеживание</bold>.',
      line2: 'Это может привести к поломке <bold>Текстовых объектов</bold> на рисунке.',
      line3:
        'Мы настоятельно рекомендуем отключить эту настройку. Для этого нужно выполнить <link>эти шаги</link>.',
      line4:
        'Если отключение этой настройки не исправит отображение текстовых объектов, создайте <issueLink>issue</issueLink> на нашем GitHub или напишите нам в <discordLink>Discord</discordLink>',
    },
    ...
    asyncPasteFailedOnRead:
      'Не удалось вставить (невозможно прочитать из системного буфера обмена).',
  ...
  r = {
    canvasPanning:
      'Чтобы двигать холст, удерживайте колесо мыши или пробел во время перетаскивания, или используйте инструмент "Рука"',
...
удерживайте кнопку ALT, чтобы изменять размер относительно центра`,
    ...
    lineEditor_info:
      'Удерживайте CtrlOrCmd и дважды кликните или нажмите CtrlOrCmd + Enter для редактирования точек',
...
или удерживайте Alt и кликните для добавления новых точек`,
    placeImage:
      'Щелкните, чтобы разместить изображение, или нажмите и перетащите, чтобы установить его размер вручную',
    publishLibrary: 'Опубликовать свою собственную библиотеку',
    bindTextToElement: 'Нажмите Enter для добавления текста',
    deepBoxSelect:
      'Удерживайте Ctrl или Cmd для глубокого выделения, чтобы предотвратить перетаскивание',
    eraserRevert: 'Удерживайте Alt, чтобы вернуть элементы, отмеченные для удаления',
    firefox_clipboard_write:
      'Эта функция может быть включена при изменении значения флага "dom.events.asyncClipboard.clipboardItem" на "true". Чтобы изменить флаги браузера в Firefox, посетите страницу "about:config".',
    disableSnapping: '',
  },
  o = {
  ...
  t = {
    ...
    clearCanvasMessage:
      'Если перезагрузка страницы не помогла, попробуйте <button>очистить холст.</button>',
    ...
    openIssueMessage:
      'Для безопасности информация о вашей сцене не включена в ошибку. Если в сцене нет ничего конфиденциального, пожалуйста следуйте нашим <button>баг трекере.</button> Пожалуйста, приложите информацию ниже, скопировав и вставив её, в issue GitHub.',
  ...
  F = {
    ...
    desc_exitSession:
      'Завершив сеанс, вы выйдете из комнаты, но сможете продолжить работать с документом локально. Это не повлияет на работу других пользователей — они смогут продолжить совместную работу с их версией документа.',
  ...
  s = {
    ...
    placeholder: {
      ...
      githubHandle:
        'Имя пользователя GitHub (необязательно), чтобы вы смогли редактировать библиотеку после её отправки на проверку',
      twitterHandle:
        'Имя пользователя в Twitter (необязательно), чтобы мы знали, кого упомянуть при продвижении в Twitter',
      website: 'Ссылка на ваш личный или какой-то другой сайт (необязательно)',
    },
    errors: { required: 'Обязательно', website: 'Введите допустимый URL-адрес' },
    noteDescription:
      'Отправить вашу библиотеку для включения в <link>хранилище публичных библиотек</link>, чтобы другие люди могли использовать объекты из вашей библиотеки в своих рисунках.',
    noteGuidelines:
      'Библиотека должна быть подтверждена вручную. Пожалуйста, прочтите <link>рекомендации</link> перед отправкой. Вам понадобится учетная запись GitHub, чтобы общаться и вносить изменения при необходимости, но это не обязательно.',
    noteLicense:
      'Выполняя отправку, вы соглашаетесь с тем, что библиотека будет опубликована под <link>лицензией MIT, </link>, что, вкратце, означает, что каждый может использовать её без ограничений.',
    noteItems:
      'Каждый объект в библиотеке должен иметь свое собственное имя, чтобы по нему можно было фильтровать. Следующие объекты библиотеки будут включены:',
    atleastOneLibItem: 'Пожалуйста, выберите хотя бы один объект в библиотеке, чтобы начать',
    republishWarning:
      'Примечание: некоторые из выбранных элементов помечены как уже опубликованные/отправленные. Вы должны повторно отправить элементы только при обновлении существующей библиотеки или сдаче работы.',
  },
  d = {
    title: 'Библиотека отправлена',
    content:
      'Благодарим вас, {{authorName}}. Ваша библиотека была отправлена на проверку. Вы можете отслеживать статус<link>здесь</link>',
  },
  c = {
    resetLibrary: 'Сброс библиотеки',
  ...
  m = {
    ...
    tooltip: {
      embedScene:
        'Сцена будет сохранена в PNG/SVG файл так, чтобы всю сцену можно будет восстановить из этого файла. Это увеличит размер файла.',
    },
    title: {
  ...
  p = {
    tooltip:
      'Ваши данные защищены сквозным (End-to-end) шифрованием. Серверы Excalidraw никогда не получат доступ к ним.',
...
или вставьте в существующий текстовый редактор`,
  ...
  w = {
    action: {
      ...
      saveToDisk: { title: 'Сохранить на диск', button: 'Сохранить на диск', description: '' },
      excalidrawPlus: { title: 'Excalidraw+', button: 'Экспорт в Excalidraw+', description: '' },
    },
    modal: {
      loadFromFile: { title: 'Загрузить из файла', button: 'Загрузить из файла', description: '' },
      shareableLink: { title: 'Загрузить по ссылке', button: '', description: '' },
    },
  },
  v = {
