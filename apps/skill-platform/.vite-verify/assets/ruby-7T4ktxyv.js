function D(e) {
  const t = e.regex,
    o = '([a-zA-Z_]\\w*[!?=]?|[-+~]@|<<|>>|=~|===?|<=>|[<>]=?|\\*\\*|[-/+%^&*~`|]|\\[\\]=?)',
    b = t.either(/\b([A-Z]+[a-z0-9]+)+/, /\b([A-Z]+[a-z0-9]+)+[A-Z]+/),
    a = t.concat(b, /(::\w+)*/),
    n = {
    ...
    s = [
      ...
      { match: [/(include|extend)\s+/, a], scope: { 2: 'title.class' }, keywords: n },
      { relevance: 0, match: [a, /\.new[. (]/], scope: { 1: 'title.class' } },
      { relevance: 0, match: /\b[A-Z][A-Z_0-9]+\b/, className: 'variable.constant' },
      { relevance: 0, match: b, scope: 'title.class' },
      { match: [/def/, /\s+/, o], scope: { 1: 'keyword', 3: 'title.function' }, contains: [l] },
      { begin: e.IDENT_RE + '::' },
      { className: 'symbol', begin: e.UNDERSCORE_IDENT_RE + '(!|\\?)?:', relevance: 0 },
      { className: 'symbol', begin: ':(?!\\s)', contains: [E, { begin: o }], relevance: 0 },
      N,
      {
        className: 'variable',
        begin: "(\\$\\W)|((\\$|@@?)(\\w+))(?=[^@$?])(?![A-Za-z])(?![@$?'])",
  ...
  const A = [
    { begin: /^\s*=>/, starts: { end: '$', contains: s } },
    {
      ...
      begin:
        ...
        '(\\w+-)?\\d+\\.\\d+\\.\\d+(p\\d+)?[^\\d][^>]+>' +
        ')(?=[ ])',
      starts: { end: '$', keywords: n, contains: s },
    },
  ];
  return (
    c.unshift(r),
