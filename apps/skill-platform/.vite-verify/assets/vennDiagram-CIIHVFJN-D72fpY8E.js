...
function ge(t, n = {}) {
  ...
  let { distances: o, constraints: a } = fe(t, e, i);
  const c = ut(o.map(ut)) / o.length;
  o = o.map((d) => d.map((b) => b / c));
  const h = (d, b) => he(d, b, o, a);
  let u = null;
  for (let d = 0; d < s; ++d) {
...
function me(t = {}) {
  ...
  function l(p) {
    ...
    E.selectAll('.venn-area path').each(function (k) {
      ...
      k.sets.length == 1 && F && !b && ((_ = !0), (R[k.sets[0]] = Ie(F)));
    });
    function P(k) {
      return (F) => {
    ...
    const L = z
      .selectAll('text')
      .filter((k) => k.sets in D)
      .text((k) => j(k))
      .attr('x', (k) => Math.floor(D[k.sets].x))
      .attr('y', (k) => Math.floor(D[k.sets].y));
    ...
    return (
      ...
      { circles: I, textCentres: D, nodes: V, enter: O, update: z, exit: C }
    );
  }
  return (
    (l.wrap = function (p) {
...
function Vt(t, n) {
  ...
  if (t.length == 1) {
    const o = t[0].circle;
    return ve(e(o.x), e(o.y), e(o.radius));
  }
  const i = [
    `
M`,
  ...
  var t = w(function (S, g, m, v) {
    ...
    M = {
      ...
      parse: w(function (g) {
        ...
        w(K, 'lex');
        for (var z, L, C, X, k = {}, F, H, et, Y; ; ) {
          if (
            ((L = v[v.length - 1]),
            this.defaultActions[L]
              ? (C = this.defaultActions[L])
              : ((z === null || typeof z > 'u') && (z = K()), (C = N[L] && N[L][z])),
          ...
          switch (C[0]) {
            ...
            case 2:
              if (
                ...
                (X = this.performAction.apply(k, [I, U, D, P.yy, C[1], p, f].concat(R))),
                typeof X < 'u')
              )
                return X;
              (H &&
...
var Me = gt,
  ...
  Ee = w((t, n) => {
    const s = it(t).sort(),
      e = {};
    for (const [i, o] of n) e[i] = _e(o) ?? o;
    mt.push({ targets: s, styles: e });
  }, 'addStyleData'),
  ze = w(() => mt, 'getStyleData'),
...
w(Bt, 'getConfig');
var Fe = w(() => {
    (Qt(), (yt.length = 0), (pt.length = 0), (mt.length = 0), bt.clear(), (vt = void 0), (It = !1));
  }, 'customClear'),
  Le = {
    getConfig: Bt,
    clear: Fe,
...
var Ve = w((t, n, s, e) => {
  ...
  const l = ot(document.createElement('div')),
    p = me()
      ...
      .height(A - g);
  l.datum(d).call(p);
  const f = u ? ee.svg(l.select('svg').node()) : void 0,
    N = ke(d, { width: M, height: A - g, padding: o?.padding ?? 15 }),
    I = new Map();
  for (const E of N) {
  ...
  const D = ne(a.background || '#f4f4f4');
  (l.selectAll('.venn-circle').each(function (E, R) {
    ...
    const L = O?.color || (D ? se(B, 30) : ie(B, 30));
    _.select('text')
...
function Ut(t, n, s, e, i, o) {
  ...
  for (const [r, y] of u.entries()) {
    ...
    const g = Math.max(80 * i, T * 2 * 0.95),
      ...
      D = Math.max(1, Math.ceil(y.length / I)),
      U = g / I,
      j = m / D;
...
var Be = { draw: Ve },
  Ke = { parser: Me, db: Le, renderer: Be, styles: Pe };
