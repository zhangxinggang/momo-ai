...
-- 工作流（画布 JSON，供 @xyflow/react 与 LangGraph 执行序列）
...
CREATE VIRTUAL TABLE IF NOT EXISTS prompts_fts USING fts5(
  title, system_prompt, user_prompt, tags,
  content='prompts', content_rowid='rowid'
);

-- FTS 触发器：插入
CREATE TRIGGER IF NOT EXISTS prompts_ai AFTER INSERT ON prompts BEGIN
  INSERT INTO prompts_fts(rowid, title, system_prompt, user_prompt, tags)
  VALUES (NEW.rowid, NEW.title, NEW.system_prompt, NEW.user_prompt, NEW.tags);
END;

-- FTS 触发器：删除
CREATE TRIGGER IF NOT EXISTS prompts_ad AFTER DELETE ON prompts BEGIN
  INSERT INTO prompts_fts(prompts_fts, rowid, title, system_prompt, user_prompt, tags)
  VALUES ('delete', OLD.rowid, OLD.title, OLD.system_prompt, OLD.user_prompt, OLD.tags);
END;
