...
import type { ISkillFileSnapshot, ISkillVersion } from '@/types/modules';
...
export function registerSkillVersionHandlers({ db }: ISkillIPCContext): void {
  ipcMain.handle(IPC_CHANNELS.SKILL_VERSION_GET_ALL, async (_, skillId: string) => {
    if (typeof skillId !== 'string' || skillId.trim().length === 0) {
      throw new Error('skill:versionGetAll requires a non-empty skillId');
    }
    return await db.getVersions(skillId);
  });

  ipcMain.handle(
    IPC_CHANNELS.SKILL_VERSION_CREATE,
    async (_, skillId: string, note?: string, filesSnapshot?: ISkillFileSnapshot[]) => {
      if (typeof skillId !== 'string' || skillId.trim().length === 0) {
      ...
      if (filesSnapshot !== undefined && !Array.isArray(filesSnapshot)) {
        throw new Error('skill:versionCreate expects filesSnapshot to be an array');
      }
      const snapshot = filesSnapshot ?? (await readCurrentFilesSnapshot(db, skillId));
      return await db.createVersion(skillId, note, snapshot);
    },
  );
  ...
  ipcMain.handle(
    ...
    async (_, skillId: string, version: number) => {
      ...
      if (typeof version !== 'number' || !Number.isFinite(version)) {
        throw new Error('skill:versionRollback requires version to be a finite number');
      }
      const targetVersion = await db.getVersion(skillId, version);
      if (!targetVersion) return null;

      const currentSkill = await db.getById(skillId);
      ...
      const updatedSkill = await db.update(skillId, {
  ...
  ipcMain.handle(
    ...
    async (_, skillId: string, versionId: string) => {
      ...
      return await db.deleteVersion(skillId, versionId);
    },
  );

  ipcMain.handle(IPC_CHANNELS.SKILL_DELETE_ALL, async (_, confirm?: boolean) => {
    // Require explicit confirmation to prevent accidental deletion
    if (confirm !== true) {
      throw new Error('skill:deleteAll requires explicit confirmation (pass true)');
    }
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.SKILL_INSERT_VERSION_DIRECT, async (_, version: ISkillVersion) => {
    if (!version || typeof version !== 'object') {
      throw new Error('skill:insertVersionDirect requires a non-null version object');
    }
    ...
    if (typeof version.skillId !== 'string' || version.skillId.trim().length === 0) {
      throw new Error('skill:insertVersionDirect requires a non-empty skill_id');
    }
    if (
    ...
    return await db.insertVersionDirect(version);
  });
}
