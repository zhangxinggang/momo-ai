...
export type EDataLayoutMigrationStatus =
  | 'already-migrated'
  | 'no-legacy-data'
  | 'migrated'
  | 'partial-failure';

export interface IDataLayoutMigrationResult {
  status: EDataLayoutMigrationStatus;
...
function writeMarker(
  ...
  const payload: ILayoutMarkerRecord = {
    version: '0.5.5',
...
function readMarker(userDataPath: string): Partial<ILayoutMarkerRecord> | null {
  const markerPath = getMarkerPath(userDataPath);
  ...
  try {
    return JSON.parse(fs.readFileSync(markerPath, 'utf8')) as Partial<ILayoutMarkerRecord>;
  } catch {
    return null;
  }
}
...
export function isDataLayoutFullyMigrated(userDataPath: string): boolean {
  ...
  try {
    const record = JSON.parse(fs.readFileSync(markerPath, 'utf8')) as Partial<ILayoutMarkerRecord>;
    const failed = record.failedEntries ?? [];
    return failed.every((entry) => !hasDataEntries(path.join(path.resolve(userDataPath), entry)));
  } catch {
    return false;
  }
}
 ...
 * a previous migration attempt did not fully complete. Logged on startup when
 * present so support can diagnose partial migrations.
 *
 * 检查根目录是否仍有旧版条目残留（说明上次迁移未完成），供启动日志诊断。
 */
...
export async function migrateLegacyDataLayout(
...
): Promise<IDataLayoutMigrationResult> {
  ...
  // 兼容历史迁移标记：若旧版曾因升级快照写入 backupId，则继续沿用；新安装不再自动生成快照。
  const canonicalBackupId = previousMarker?.backupId ?? null;
