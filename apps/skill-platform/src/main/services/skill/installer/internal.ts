...
export async function normalizeExistingPath(absolutePath: string): Promise<string> {
...
export function validateSkillName(skillName: string): void {
  ...
  if (skillName.includes('..') || skillName.includes('/') || skillName.includes('\\')) {
    throw new Error(`Invalid skill name: must not contain "..", "/" or "\\": ${skillName}`);
  }
  // Reject absolute paths on Windows (e.g., C:\)
  if (/^[a-zA-Z]:/.test(skillName)) {
    throw new Error(`Invalid skill name: must not be an absolute path: ${skillName}`);
  }
  ...
  if (path.dirname(resolved) !== path.resolve(skillsDir)) {
    throw new Error(`Invalid skill name: resolved path escapes skills directory: ${skillName}`);
  }
}
...
export function validateRelativePath(relativePath: string): void {
  ...
  if (relativePath.includes('..')) {
    throw new Error(`Invalid relative path: must not contain "..": ${relativePath}`);
  }
  if (relativePath.startsWith('/') || relativePath.startsWith('\\')) {
    throw new Error(`Invalid relative path: must not start with "/" or "\\": ${relativePath}`);
  }
  // Reject absolute paths on Windows (e.g., C:\)
  if (/^[a-zA-Z]:/.test(relativePath)) {
    throw new Error(`Invalid relative path: must not be an absolute path: ${relativePath}`);
  }
}
...
export async function resolveRepoBasePath(
  ...
  const realResolvedBasePath = await fs.realpath(resolvedBasePath).catch(() => resolvedBasePath);
  ...
  if (
    ...
    throw new Error('Path traversal detected: base path is outside skills directory');
  }
...
export async function resolveRepoTargetPath(
  ...
  const { resolvedBasePath, realBasePath } = await resolveRepoBasePath(absoluteBasePath, {
