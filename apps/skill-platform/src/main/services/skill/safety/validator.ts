 ...
 * 解析 SKILL.md 结果
 */
export interface IParsedSkillMd {
 ...
 * 验证技能名称格式
 *
 * @param name - ISkill name to validate
...
export function getSkillNameError(name: string): string | null {
  if (!name || typeof name !== 'string') {
    return 'ISkill name is required';
  }

  if (name.length < 1) {
    return 'ISkill name cannot be empty';
  }

  if (name.length > 64) {
    return 'ISkill name cannot exceed 64 characters';
  }

  if (!SKILL_NAME_REGEX.test(name)) {
    if (name !== name.toLowerCase()) {
      return 'ISkill name must be lowercase';
    }
    if (name.startsWith('-') || name.endsWith('-')) {
      return 'ISkill name cannot start or end with a hyphen';
    }
    if (name.includes('--')) {
      return 'ISkill name cannot contain consecutive hyphens';
    }
    if (/[^a-z0-9-]/.test(name)) {
      return 'ISkill name can only contain lowercase letters, numbers, and hyphens';
    }
 ...
 */
export function parseSkillMd(content: string): IParsedSkillMd | null {
  ...
  const frontmatter: ISkillFrontmatter = { name: '' };
 ...
 */
export function validateSkillMd(content: string, directoryName?: string): IValidationResult {
  const errors: string[] = [];
  ...
  if (!parsed.frontmatter.name) {
    ...
    if (directoryName && parsed.frontmatter.name !== directoryName) {
      warnings.push(
        `ISkill name "${parsed.frontmatter.name}" does not match directory name "${directoryName}"`,
      );
    }
  }
