...
export function buildSkillSyncUpdateFromRepo(
  ...
  if (sanitized.author !== undefined && sanitized.author !== (skill.author ?? undefined)) {
    update.author = sanitized.author;
    changed = true;
  }

  if (sanitized.version !== undefined && sanitized.version !== (skill.version ?? undefined)) {
 ...
 */
const METADATA_KEYS: (keyof DUpdateSkill)[] = ['description', 'author', 'name', 'tags', 'version'];

export function hasMetadataChanges(data: DUpdateSkill): boolean {
  return METADATA_KEYS.some((key) => Object.prototype.hasOwnProperty.call(data, key));
}
...
export async function syncFrontmatterToRepo(
  ...
  // Write back
  await SkillInstaller.writeLocalRepoFileByPath(repoPath, 'SKILL.md', newContent);
}
