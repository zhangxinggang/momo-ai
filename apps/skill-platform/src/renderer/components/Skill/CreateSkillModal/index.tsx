...
export function CreateSkillModal({ isOpen, onClose }: IProps) {
  ...
  const hideMainModal = modal.isDefaultMode && modal.showDefaultPreview;
  ...
  return (
    <>
        ...
        <Modal
          ...
          <div
            className={`p-6 ${
              modal.isManualMode
                ? ''
                : modal.isGitHubMode || modal.isScanMode || modal.isDefaultMode
      ...
      {modal.showDefaultPreview && (
        <SkillScanPreview
          variant='default-import'
          scannedSkills={modal.defaultScanResults}
          installedNames={modal.installedDefaultNames}
          isLoading={modal.isLoadingDefault}
          loadError={modal.error}
          onImport={modal.handleDefaultImport}
          onClose={modal.handleCloseDefaultPreview}
        />
      )}
      <UnsavedLeaveDialog />
    </>
