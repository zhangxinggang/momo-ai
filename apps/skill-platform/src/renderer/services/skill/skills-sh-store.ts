...
export type ESkillsShFilterKey = (typeof SKILLS_SH_FILTERS)[number]['key'];
...
export interface ISkillsShLeaderboardEntry {
  owner: string;
  repo: string;
  skillName: string;
  detailPath: string;
...
export function normalizeSkillsShFilterKey(value?: string | null): ESkillsShFilterKey {
  return SKILLS_SH_FILTERS.some((filter) => filter.key === value)
    ? (value as ESkillsShFilterKey)
    : 'all';
}
...
export function getSkillsShIndexUrl(filterKey?: string | null): string {
  ...
  const filter =
    SKILLS_SH_FILTERS.find((item) => item.key === normalizedFilterKey) ?? SKILLS_SH_FILTERS[0];
...
function getStandardSkillsShPackageLocation(entry: ISkillsShLeaderboardEntry):
  | {
...
export function parseSkillsShLeaderboard(
...
): ISkillsShLeaderboardEntry[] {
  const entries: ISkillsShLeaderboardEntry[] = [];
  const seen = new Set<string>();
  const limit = options?.limit ?? 24;
  const normalizedHtml = html.replace(/\\"/g, '"');
  const linkPattern = /<a[^>]+href="(\/[^"/?#]+\/[^"/?#]+\/[^"/?#]+\/?)"[^>]*>([\s\S]*?)<\/a>/gi;

  const addEntry = (entry: ISkillsShLeaderboardEntry) => {
  ...
  const dataPattern = /"source":"([^"]+\/[^"]+)","skillId":"([^"]+)","name":"([^"]+)"/g;
...
export function filterSkillsShLeaderboardEntries(
  entries: readonly ISkillsShLeaderboardEntry[],
  query: string,
): ISkillsShLeaderboardEntry[] {
  ...
  return entries.filter((entry) => {
    const haystack = normalizeSearchTerm(
      [entry.owner, entry.repo, entry.skillName, entry.detailPath, entry.detailUrl].join(' '),
    );
...
export function parseSkillsShDetail(
  html: string,
  entry: ISkillsShLeaderboardEntry,
): IRegistrySkill | null {
  ...
  const repository = extractSimpleMetric(text, 'Repository') || `${entry.owner}/${entry.repo}`;
  const weeklyInstalls = extractSimpleMetric(text, 'Weekly Installs') || entry.weeklyInstalls;
  ...
  const displayName = frontmatter.name?.trim() || humanizeSkillName(entry.skillName);
  const description =
    summary || frontmatter.description?.trim() || `${displayName} community skill`;
  const compatibility =
...
export function getRegistrySkillDirectory(
  regSkill: Pick<IRegistrySkill, 'source_directory' | 'canonical_skill_path'>,
): string | undefined {
  const explicitDirectory = regSkill.source_directory?.trim().replace(/^\/+|\/+$/g, '');
  if (explicitDirectory) {
    return explicitDirectory;
  }

  const canonicalPath = regSkill.canonical_skill_path?.trim().replace(/^\/+|\/+$/g, '');
...
export function isSkillsShRegistrySkill(regSkill: IRegistrySkill): boolean {
  const sourceText = [regSkill.source_label, regSkill.store_url, regSkill.source_url]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
  return sourceText.includes('skills.sh');
}
