...
export function ChatFeatureDropdown({
  ...
  const panel = customPanel ? (
    <div className='chat-feature-dropdown-panel chat-feature-dropdown-panel--custom min-w-[280px] max-w-[320px] overflow-hidden px-3 py-2'>
      {customPanel}
    </div>
  ) : (
    <div className='chat-feature-dropdown-panel min-w-[240px] max-w-[280px] overflow-hidden py-1'>
      ...
      {enabled && showExtraWhenEnabled && children ? (
        <div className='border-surface max-h-[220px] overflow-hidden border-t px-3 py-2'>
          {children}
        </div>
      ) : null}
    </div>
