    ...
    const workspaceExtra = workspace ? (
      <div className='flex items-center justify-between gap-2'>
        <span
          ...
          {workspace.path ? formatWorkspaceDisplayPath(workspace.path) : '未选择目录'}
        </span>
        <Button
          type='link'
          size='small'
          className='shrink-0 px-0 text-xs'
          onClick={workspace.onSelectFolder}>
          {workspace.path ? '更换' : '选择'}
        </Button>
      </div>
    ) : null;
          ...
          <div className='px-4 pt-4'>
            <div className='flex flex-nowrap items-stretch gap-3 overflow-x-auto'>
              {attachments.map((f) => {
                ...
                const iconBase = attachmentIconBasePath;
                const iconSrc = (() => {
                  if (extLower === 'docx') return `${iconBase}word-icon.svg`;
                  if (extLower === 'md' || extLower === 'txt') return `${iconBase}txt-icon.svg`;
                  if (extLower === 'css') return `${iconBase}css-icon.svg`;
                  if (extLower === 'html') return `${iconBase}html-icon.svg`;
                  if (extLower === 'js') return `${iconBase}js-icon.svg`;
                  if (extLower === 'py') return `${iconBase}py-icon.svg`;
                  return `${iconBase}txt-icon.svg`;
                })();
                ...
                return (
                  <div
                    ...
                    <img src={iconSrc} alt={displayType} className='mt-0 h-8 w-8 select-none' />
