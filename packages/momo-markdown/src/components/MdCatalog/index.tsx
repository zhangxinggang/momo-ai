...
import { CatalogContext, ICatalogContextValue } from './context';
...
const MdCatalog = (props: IProps) => {
  ...
  const [list, setList] = useState<Array<IHeadList>>([]);

  const [activeItem, setActiveItem] = useState<IHeadList>();
  ...
  const onActive = useCallback(
    (tocItem: ITocItem, ele: HTMLDivElement) => {
      setIStyles({
  ...
  useEffect(() => {
    let cacheList: IHeadList[] = [];
    const findActiveHeading = (list_: IHeadList[]) => {
      if (list_.length === 0) {
      ...
      const { activeHead, activeIndex } = list_.reduce(
        (activeData, link, index) => {
          ...
          if (syncWith === 'preview') {
            ...
            if (editorView) {
              const top = editorView.lineBlockAt(editorView.state.doc.line(link.line + 1).from).top;
    ...
    const callback = (_list: Array<IHeadList>) => {
  ...
  }, [offsetTop, mdHeadingId, getScrollElement, editorId, syncWith, editorView, catalogMaxDepth]);

  const providerValue = useMemo<ICatalogContextValue>(() => {
    return {
      scrollElementRef,
      rootNodeRef,
    };
