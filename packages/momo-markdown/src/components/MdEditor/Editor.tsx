...
const Editor = forwardRef((props: IEditorProps, ref: ForwardedRef<unknown>) => {
  ...
  const [staticProps] = useState<IStaticProps>(() => {
    return {
      editorId,
      noKatex,
      noMermaid,
  ...
  const codeRef = useRef<IContentExposeParam>(undefined);
  ...
  const contextValue = useMemo<IContextType>(() => {
    return {
