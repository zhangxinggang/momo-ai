...
import { codeCss, defaultProps, globalConfig, prefix, staticTextDefault } from './config';
 ...
 * @param staticProps
 */
export const useOnSave = (props: IEditorProps, staticProps: IStaticProps) => {
  ...
  }, [editorId, onSave, state.buildFinished, state.html, value]);
 ...
 * @param staticProps
 */
export const useExpansion = (staticProps: IStaticProps) => {
 ...
 * @param onError
 */
export const useErrorCatcher = (editorId: string, onError: (err: IInnerError) => void) => {
  useEffect(() => {
    bus.on(editorId, {
      name: ERROR_CATCHER,
      callback: onError,
 ...
 * @param staticProps
 */
export const useUploadImg = (props: IEditorProps, staticProps: IStaticProps) => {
  const { editorId } = staticProps;
  const { onUploadImg } = props;

  useEffect(() => {
    const uploadImageCallBack = (files: Array<File>, cb: () => void) => {
      const insertHanlder: TUploadImgCallBack = (urls) => {
 ...
 * @returns
 */
export const useCatalog = (_props: IEditorProps, staticProps: IStaticProps) => {
  const { editorId } = staticProps;
...
export const useMdPreviewConfig = (props: IMdPreviewProps) => {
  const {
  ...
  const highlight = useMemo(() => {
    ...
    const _theme =
      codeStyleReverse && codeStyleReverseList.includes(previewTheme) ? 'dark' : (theme as TThemes);

    // 找到对应代码主题的链接和属性
    const codeCssHref = cssList[codeTheme] ? cssList[codeTheme][_theme] : codeCss.atom[_theme];
    const codeCssAttrs =
  ...
  // 缓存语言设置
  const usedLanguageText = useMemo<IStaticTextDefaultValue>(() => {
    const allText: any = {
      ...staticTextDefault,
...
export const useConfig = (props: IEditorProps): [any, any, ISettingType, TUpdateSetting] => {
  ...
  const updateSetting = useCallback<TUpdateSetting>((k, v) => {
    setSetting((_setting) => {
      const realValue = v === undefined ? !_setting[k] : v;

      const nextSetting: ISettingType = {
        ..._setting,
      };
        ...
        switch (eventName) {
          ...
          case 'fullscreen': {
            bus.on(editorId, {
              ...
              callback(status: boolean) {
                (callBack as IExposeEvent['fullscreen'])(status);
              },
            });
          ...
          case 'preview': {
            bus.on(editorId, {
              ...
              callback(status: boolean) {
                (callBack as IExposeEvent['preview'])(status);
              },
            });
          ...
          case 'previewOnly': {
            bus.on(editorId, {
              ...
              callback(status: boolean) {
                (callBack as IExposeEvent['previewOnly'])(status);
              },
            });
          ...
          case 'htmlPreview': {
            bus.on(editorId, {
              ...
              callback(status: boolean) {
                (callBack as IExposeEvent['htmlPreview'])(status);
              },
            });
          ...
          case 'catalog': {
            bus.on(editorId, {
              ...
              callback(status: boolean) {
                (callBack as IExposeEvent['catalog'])(status);
              },
            });
...
export const useEditorId = (props: IMdPreviewProps) => {
  const defaultId = useId();
  return props.id || prefix + '-' + defaultId.replaceAll(':', '');
};
