...
export interface IFloatingToolbarContextValue<T = unknown> {
  getValue: () => T;
  subscribe: (callback: () => void) => () => void;
}

export const FloatingToolbarContext = createContext<IFloatingToolbarContextValue<IContextType>>({
...
export const createFloatingToolbar = (options: {
  contextValue: IFloatingToolbarContextValue<IContextType>;
}) => {
  ...
  const showTooltip = (view: EditorView, nextState: TooltipState) => {
    if (lastTooltip && lastTooltip.kind === nextState.kind && lastTooltip.pos === nextState.pos) {
      return;
    }
