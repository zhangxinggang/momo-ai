...
const useAutoScroll = (
  ...
  // 更新完毕后判断是否需要重新绑定滚动事件
  useEffect(() => {
    const rootNode = codeMirrorUt.current?.view.contentDOM.getRootNode() as Document | ShadowRoot;
    const cmScroller = rootNode?.querySelector<HTMLDivElement>(`#${editorId} .cm-scroller`);

    const previewEle = rootNode?.querySelector<HTMLElement>(`[id="${editorId}-preview-wrapper"]`);
    const htmlEle = rootNode?.querySelector<HTMLElement>(`[id="${editorId}-html-wrapper"]`);
  ...
  }, [scrollCb, props.scrollAuto, setting.preview, setting.htmlPreview, setting.previewOnly]);
};
