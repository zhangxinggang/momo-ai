...
import { ICodeMirrorExtension, TDOMEventHandlers, TThemes } from '~/type';
import { directive2flag, TToolDirective } from '~/utils/content-help';
...
import { createTextShortener, ITextShortenerOptions } from '../codemirror/textShortener';
...
const getSourceExtension = (item: ICodeMirrorExtension) => {
  return item.extension instanceof Function ? item.extension(item.options) : item.extension;
};

const produceExtension = (item: ICodeMirrorExtension): Extension => {
 ...
 * @returns
 */
const useCodeMirror = (props: IContentProps) => {
  const contextValue = useContext(EditorContext);
  const { tabWidth, editorId, theme, noPrettier, disabled, floatingToolbars } = contextValue;
  ...
  const [domEventHandlersUserDefined, setDEHUD] = useState<TDOMEventHandlers>({});

  const domEventHandlers = useMemo<TDOMEventHandlers>(() => {
    const basicHandlers: TDOMEventHandlers = {
      paste: pasteHandler,
      blur: props.onBlur,
      focus: props.onFocus,
      drop: props.onDrop,
    ...
    const nextDomEventHandlers: TDOMEventHandlers = {
      ...basicHandlers,
    };
    ...
    for (const eventName in domEventHandlersUserDefined) {
      ...
      if (defaultEventNames.includes(en)) {
        nextDomEventHandlers[en] = (e, v) => {
          (domEventHandlersUserDefined[en] as (event: Event, view: EditorView) => void)(e, v);
  ...
  const [defaultExtensions] = useState(() => {
    return [
      {
        type: 'theme',
        extension: ({ theme }: { theme: TThemes }) => (theme === 'light' ? oneLight : oneDark),
  ...
  const [userDefindeExtension] = useState(() => {
    return globalConfig.codeMirrorExtensions(
      [
        ...
        {
          type: 'linkShortener',
          extension: (options: any) => createTextShortener(options as ITextShortenerOptions),
          options: {
            maxLength: 30,
          },
        },
  ...
  useEffect(() => {
    ...
    const eventListener = (handlers: TDOMEventHandlers) => {
      setDEHUD(handlers);
    };
  ...
  useEffect(() => {
    const callback = async (direct: TToolDirective, params = {} as any) => {
      ...
      if (direct === 'image' && params.transform) {
        ...
        const { text, options } = await directive2flag(direct, codeMirrorUt.current!, params);
        codeMirrorUt.current?.replaceSelectedText(text as string, options, editorId);
      }
    };
    // 注册指令替换内容事件
