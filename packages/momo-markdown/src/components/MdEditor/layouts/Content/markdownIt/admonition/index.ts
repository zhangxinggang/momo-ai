...
import { markdownit, type MdRenderRule } from '~~/types/markdown-it-compat';

export interface IAdmonitionPluginOps {
  marker?: string;
}

const AdmonitionPlugin = (md: markdownit, options: IAdmonitionPluginOps) => {
  options = options || {};
  ...
  const render: MdRenderRule = (tokens, idx, _options, _env, self) => {
