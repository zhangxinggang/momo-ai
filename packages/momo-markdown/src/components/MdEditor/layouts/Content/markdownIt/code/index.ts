...
import { ICustomIcon, IStaticTextDefaultValue } from '~/type';
import { mergeAttrs } from '~/utils/md-it';
import { markdownit, type MdRenderer, type MdToken } from '~~/types/markdown-it-compat';

import type { MdItOptions } from '~~/types/markdown-it-compat';

export interface ICodeTabsPluginOps extends MdItOptions {
  editorId: string;
  usedLanguageTextRef: RefObject<IStaticTextDefaultValue>;
  codeFoldable: boolean;
  autoFoldThreshold: number;
  customIconRef: RefObject<ICustomIcon>;
  extraTools?: string | (({ lang }: { lang: string }) => string);
}

const codetabs = (md: markdownit, _opts: ICodeTabsPluginOps) => {
  const defaultRender = md.renderer.rules.fence,
    unescapeAll = md.utils.unescapeAll,
    re = /\[(\w*)(?::([\w ]*))?\]/,
    mandatoryRe = /::(open|close)/;

  const getInfo = (token: MdToken) => {
    return token.info ? unescapeAll(token.info).trim() : '';
  };

  const getGroupAndTab = (token: MdToken) => {
  ...
  const getLangName = (token: MdToken) => {
    const info = getInfo(token);
    return info ? info.split(/(\s+)/g)[0] : '';
  };

  const getTagType = (token: MdToken) => {
  ...
  const fenceGroup = (
    ...
    if (GROUP === null) {
      ...
      return `
        <${tagContainer} ${slf.renderAttrs(tmpMdToken as MdToken)}>
