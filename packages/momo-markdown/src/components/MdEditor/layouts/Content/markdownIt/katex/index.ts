...
const create_math_inline =
  (options: ICreateOptions): MdRuleInline =>
  (state, silent) => {
...
const create_math_block =
  (options: ICreateOptions): MdRuleBlock =>
  (state, start, end, silent) => {
    ...
    for (const delim of delimiters) {
      ...
      // 确保开始符号在行首
      if (state.src.slice(contentStart, contentStart + delim.open.length) !== delim.open) {
        continue;
      }
...
const KatexPlugin = (
  md: markdownit,
  { katexRef, inlineDelimiters, blockDelimiters }: IKatexOptions,
) => {
  const renderKatex = (
    token: MdToken,
    className: string,
