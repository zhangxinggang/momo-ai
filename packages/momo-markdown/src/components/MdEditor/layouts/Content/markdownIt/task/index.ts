 ...
 */
import { markdownit, type MdToken } from '~~/types/markdown-it-compat';

interface ITaskListPluginOptions {
  enabled?: boolean;
  label?: boolean;
  labelAfter?: boolean;
}

const attrSet = (token: MdToken, name: string, value: string) => {
...
const isInline = (token: MdToken) => {
  return token.type === 'inline';
};

const isParagraph = (token: MdToken) => {
  return token.type === 'paragraph_open';
};

const isListItem = (token: MdToken) => {
  return token.type === 'list_item_open';
};

const startsWithTodoMarkdown = (token: MdToken) => {
...
const isTodoItem = (tokens: MdToken[], index: number) => {
  return (
...
const parentToken = (tokens: MdToken[], index: number) => {
...
const beginLabel = (TokenConstructor: any): MdToken => {
  const token = new TokenConstructor('html_inline', '', 0);
  token.content = '<label>';
  return token;
};

const endLabel = (TokenConstructor: any): MdToken => {
  const token = new TokenConstructor('html_inline', '', 0);
  token.content = '</label>';
  return token;
};

const afterLabel = (content: string, id: string, TokenConstructor: any): MdToken => {
  const token = new TokenConstructor('html_inline', '', 0);
...
const makeCheckbox = (
  ...
  options: ITaskListPluginOptions,
): MdToken => {
...
const todoify = (token: MdToken, TokenConstructor: any, options: ITaskListPluginOptions) => {
...
const githubTaskLists = (md: markdownit, options: ITaskListPluginOptions = {}) => {
