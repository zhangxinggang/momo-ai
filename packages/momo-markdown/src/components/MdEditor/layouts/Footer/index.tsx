import { cloneElement, memo, ReactElement, useCallback, useContext, useMemo } from 'react';
import { allFooter, prefix } from '~/config';
import { EditorContext } from '~/context';
import { TFooters } from '~/type';
...
const Footer = (props: IProps) => {
  ...
  const [LeftFooter, RightFooter] = useMemo(() => {
    ...
    const barRight =
      moduleSplitIndex === -1 ? [] : props.footers.slice(moduleSplitIndex, Number.MAX_SAFE_INTEGER);

    return [barLeft.map((name) => footerRender(name)), barRight.map((name) => footerRender(name))];
