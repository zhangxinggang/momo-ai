...
import { TTableShapeType } from '~/type';
...
interface IProps {
  ...
  onSelected: (data: IHoverData) => void;
}

const TableShape = (props: IProps) => {
  const [hoverPosition, setHoverPosition] = useState<IHoverData>({
    x: -1,
    y: -1,
  });
          ...
          {new Array(tableShape[0]).fill('').map((_, colIndex) => (
            <div
              ...
              onMouseEnter={() => {
                ...
                } else if (colIndex + 2 < tableShape[0] && tableShape[0] > props.tableShape[0]) {
                  setTableShape((ts) => {
                    const shapeCopy = [...ts];
                    shapeCopy[0] = ts[0] - 1;
                    return shapeCopy;
                ...
                } else if (rowIndex + 2 < tableShape[1] && tableShape[1] > props.tableShape[1]) {
                  setTableShape((ts) => {
                    const shapeCopy = [...ts];
                    shapeCopy[1] = ts[1] - 1;
                    return shapeCopy;
