...
const handlePrettier = async (codeMirrorUt: CodeMirrorUt, params: any) => {
  const prettier = window.prettier || globalConfig.editorExtensions.prettier?.prettierInstance;
...
const handleImage = (params: any, codeMirrorUt: CodeMirrorUt) => {
  ...
  if (urls instanceof Array) {
    text = (urls as TUploadImgCallBackParam).reduce<string>((pVal, _url) => {
      const { url = '', alt = '', title = '' } = typeof _url === 'object' ? _url : { url: _url };
...
const handleUniversal = (selectedText: string, params: any) => {
  const { generate } = params as { generate: TInsertContentGenerator };
...
const getSelectedInfo = (
  ...
  return [state.doc.sliceString(selection.from, selection.to), selection.from, selection.to];
};
