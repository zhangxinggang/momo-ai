...
import { useEditorId, useMdPreviewConfig } from '~/hooks';
import ContentPreview from '~/layouts/Content/ContentPreview';
import { IContextType, IMdPreviewProps, IMdPreviewStaticProps, TThemes } from '~/type';
import { classnames } from '~/utils';
import bus from '~/utils/event-bus';

import { useExpose } from './hooks/useExpose';

const MdPreview = forwardRef((props: IMdPreviewProps, ref: ForwardedRef<unknown>) => {
  // Editor.defaultProps在某些编辑器中不能被正确识别已设置默认情况
  const {
    value = props.value ?? defaultProps.value,
    onChange = defaultProps.onChange,
    theme = defaultProps.theme as TThemes,
  ...
  const [staticProps] = useState<IMdPreviewStaticProps>(() => {
    return {
      editorId,
      noKatex,
      noMermaid,
  ...
  const providerValue = useMemo<IContextType>(() => {
    return {
