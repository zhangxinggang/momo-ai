...
const MERMAID_CLASS = `${EXPORT_EDITOR_PREFIX}-mermaid`;
const ECHARTS_CLASS = `${EXPORT_EDITOR_PREFIX}-echarts`;
const PLANTUML_RENDERED_CLASS = `${EXPORT_EDITOR_PREFIX}-plantuml-rendered`;
const PLANTUML_IMAGE_CLASS = `${EXPORT_EDITOR_PREFIX}-plantuml-image`;

const HEADING_TAG_LEVEL: Record<string, (typeof HeadingLevel)[keyof typeof HeadingLevel]> = {
...
function createParagraph(options: IParagraphOptions = {}): Paragraph {
...
function createTextRun(text: string, style: IInlineStyle = {}): TextRun {
  return new TextRun({
    text,
    bold: style.bold,
...
function buildInlineRuns(node: Node, style: IInlineStyle = {}): ParagraphChild[] {
...
function buildParagraphFromElement(
  element: HTMLElement,
  options: {
