...
import type {
  ...
  IRouteStackItem,
  TControllerExports,
  TControllerMethodMap,
  TNext,
  TProxyKoaContext,
  TRouteHandler,
  TSendKoaContext,
  TSendOptions,
} from '../../types/http-server/routes';
import type { IHttpRouteDirConfig, IHttpRoutesConfig } from '../../types/runtime-config';
import authority from './authority';

const isRouteHandlerArray = (value: unknown): value is TRouteHandler[] => {
...
class Routers {
  public routes: IRouteLike[];
  public proxyRoutes?: IHttpRoutesConfig['proxyRoutes'];
  public staticDirs?: IHttpRouteDirConfig[];
  public mountRouteDirs?: IHttpRouteDirConfig[];
  public dynamicRouteDirs?: IHttpRouteDirConfig[];
  ...
  formatterRootPath(item: IHttpRouteDirConfig): void {
  ...
  routeExistCheck(app: IAppLike): void {
    app.use(async (ctx: IDynamicCtx, next: TNext) => {
      let exist = this.routes.some((item) => {
        return item.stack.some((rt: IRouteStackItem) => {
  ...
  routeAuthCheck(app: IAppLike): void {
  ...
  loadProxyRoutes(): void {
    ...
    Object.keys(this.proxyRoutes).forEach((item) => {
      ...
      let routes = () => {
        return async function (ctx: IDynamicCtx, next: TNext): Promise<void> {
          if (regexp.test(url.parse(ctx.url).pathname)) {
            ...
            await koaConnect(createProxyMiddleware(item, proxyOptions))(
              ctx as unknown as TProxyKoaContext,
              next,
            );
          } else {
            await next();
  ...
  loadStaticRoutes(): void {
    ...
    this.staticDirs.map((item) => {
      ...
      let routes = () => {
        let opts = Object.assign({}, item) as IHttpRouteDirConfig & {
          root?: string;
        };
        opts.root = opts.rootDir;
        if (opts.index !== false) opts.index = opts.index || 'index.html';
        return async function staticServe(ctx: IDynamicCtx, next: TNext): Promise<void> {
          let done = false;
          if (methods.includes(ctx.method)) {
            try {
              done = await send(
                ctx as unknown as TSendKoaContext,
                normalizeSafe(ctx.path.replace(opts.rootPath, '/')),
                opts as TSendOptions,
              );
            } catch (err: unknown) {
  ...
  loadMountRoutes(): void {
    ...
    this.mountRouteDirs.map((item) => {
      ...
      glob.sync(join(item.rootDir, '/**/*.js'), { ignore: item.ignore }).map((file) => {
        const controllers = require(file) as TControllerExports;
        _.forOwn(controllers, (value: unknown, key: string) => {
          ...
          if (isRouteHandlerArray(handlers)) {
            ...
            _.forOwn(handlers as TControllerMethodMap, (funcs: unknown, method: string) => {
              ...
              (
                router as unknown as Record<
                  string,
                  (path: string, ...args: TRouteHandler[]) => void
                >
              )[method](key, ...funcs);
            });
          }
      ...
      glob.sync(join(item.rootDir, '/**/*.{m}.*js'), { ignore: item.ignore }).map((filterRoute) => {
        ...
        if (routeType.length > 1) {
          ...
          (
            router as unknown as Record<
              string,
              (path: string, handler: (ctx: IDynamicCtx) => Promise<void>) => void
            >
          )[method](combinedRoute, async (ctx: IDynamicCtx) => {
            try {
  ...
  async standardResponse(ctx: IDynamicCtx, next: () => Promise<void>): Promise<void> {
    const res = (ctx: IDynamicCtx, data: unknown = [], status?: number, msg?: string): void => {
      let returnInfo = {
        data,
        meta: {
          status,
    ...
    const success = (ctx: IDynamicCtx, data: unknown, status = 200, message = 'success'): void => {
      res(ctx, data, status, message);
    };
    const error = (ctx: IDynamicCtx, status?: unknown, message?: string): void => {
      let eStatus = 400;
      let eMessage = null;
