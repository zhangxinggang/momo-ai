 ...
 */
import type { IAMFAnyResult, IAMFObject, IStrictArrayLike } from '../../types/rtmp-server/amf';
const amf3dRules = {
...
function amfType(o) {
  ...
  if (jsType == 'object') {
    if (o instanceof Array) {
      if ((o as IStrictArrayLike).sarray) return 'sarray';
      return 'array';
    }
    return 'object';
  }
 ...
 */
function amf3decString(buf): IAMFAnyResult {
  const sLenInfo = amf3decUI29(buf);
  let s = sLenInfo.value & 1;
  let sLen = sLenInfo.value >> 1; // The real length without the lowest bit
  if (s)
    return {
      len: sLen + 5,
      value: buf.slice(5, sLen + 5).toString('utf8'),
    };
 ...
 * @returns {{len: *, value: (*|String)}}
 */
function amf3decXmlDoc(buf): IAMFAnyResult {
  const sLenInfo = amf3decUI29(buf);
  let s = sLenInfo.value & 1;
  let sLen = sLenInfo.value >> 1; // The real length without the lowest bit
  if (s)
 ...
 * @returns {{len: *, value: (*|String)}}
 */
function amf3decXml(buf): IAMFAnyResult {
  const sLenInfo = amf3decUI29(buf);
  let s = sLenInfo.value & 1;
  let sLen = sLenInfo.value >> 1; // The real length without the lowest bit
  if (s)
 ...
 */
function amf3decByteArray(buf): IAMFAnyResult {
  const sLenInfo = amf3decUI29(buf);
  let s = sLenInfo.value & 1; // TODO: Check if we follow the same rule!
  let sLen = sLenInfo.value >> 1; // The real length without the lowest bit
  if (s) return { len: sLen + 5, value: buf.slice(5, sLen + 5) };
  throw new Error('Error, we have a need to decode a String that is a Reference'); // TODO: Implement references!
}
 ...
 * @returns {{len: *, value: *}}
 */
function amf3decArray(buf): IAMFAnyResult {
 ...
 * @param buf
 */
function amf3decObject(buf): IAMFAnyResult {
  let obj: Record<string, unknown> = {};
  let pos = 0;
  return { len: 0, value: obj };
}
 ...
 */
function amf0decObject(buf): IAMFAnyResult {
...
function amf0decTypedObj(buf) {
  ...
  (obj.value as Record<string, unknown>).__className__ = className.value;
...
function amfXDecode(rules, buffer) {
  ...
  for (let i = 0; i < buffer.length; ) {
    res = amfXDecodeOne(rules, buffer.slice(i)) as IAMFAnyResult;
    i += res.len;
 ...
 */
function decodeAmf0Data(dbuf: Buffer): IAMFObject {
  let buffer = dbuf;
 ...
 * @returns {{cmd: (*|string|String|*), value: *}}
 */
function decodeAMF0Cmd(dbuf: Buffer): IAMFObject {
  let buffer = dbuf;
  let resp: Record<string, unknown> = {};
 ...
 * @returns {*}
 */
function encodeAMF0Cmd(opt: IAMFObject): Buffer {
  let data = amf0EncodeOne(opt.cmd);
  const cmd = String(opt.cmd);

  if (rtmpCmdCode[cmd]) {
    rtmpCmdCode[cmd].forEach(function (n) {
...
function encodeAMF0Data(opt: IAMFObject): Buffer {
  let data = amf0EncodeOne(opt.cmd);
  const cmd = String(opt.cmd);

  if (rtmpDataCode[cmd]) {
    rtmpDataCode[cmd].forEach(function (n) {
      if (opt.hasOwnProperty(n)) data = Buffer.concat([data, amf0EncodeOne(opt[n])]);
    });
  } else {
 ...
 * @returns {{}}
 */
function decodeAMF3Cmd(dbuf: Buffer): IAMFObject {
  let buffer = dbuf;
  let resp: Record<string, unknown> = {};
 ...
 * @returns {*}
 */
function encodeAMF3Cmd(opt: IAMFObject): Buffer {
  let data = amf0EncodeOne(opt.cmd);
  const cmd = String(opt.cmd);
