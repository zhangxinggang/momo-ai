...
import Bitop = require('./node_core_bitop');
import type {
  IAACInfo,
  IAVCInfo,
  IHEVCInfo,
  IHEVCPtlInfo,
  IHEVCSpsInfo,
} from '../../types/rtmp-server/av-codec';
...
function getObjectType(bitop: Bitop): number {
...
function getSampleRate(bitop: Bitop, info: IAACInfo): number {
  info.sampling_index = bitop.read(4);
  return info.sampling_index == 0x0f ? bitop.read(24) : AAC_SAMPLE_RATE[info.sampling_index];
}

function readAACSpecificConfig(aacSequenceHeader: Buffer): IAACInfo {
  let info = {} as IAACInfo;
...
function getAACProfileName(info: IAACInfo) {
...
function readH264SpecificConfig(avcSequenceHeader: Buffer): IAVCInfo {
  let info = {} as IAVCInfo;
  let profile_idc,
    width,
    height,
    crop_left,
...
function HEVCParsePtl(bitop: Bitop, hevc: IHEVCInfo, max_sub_layers_minus1: number): IHEVCPtlInfo {
  let general_ptl = {} as IHEVCPtlInfo;
...
function HEVCParseSPS(SPS: Buffer, hevc: IHEVCInfo): IHEVCSpsInfo {
  let psps = {} as IHEVCSpsInfo;
  ...
  psps.profile_tier_level = HEVCParsePtl(rbspBitop, hevc, psps.sps_max_sub_layers_minus1 || 0);
  ...
  if (psps.conformance_window_flag) {
    let vert_mult = 1 + Number((psps.chroma_format_idc || 0) < 2);
    let horiz_mult = 1 + Number((psps.chroma_format_idc || 0) < 3);
...
function readHEVCSpecificConfig(hevcSequenceHeader: Buffer): IAVCInfo {
  let info = {} as IAVCInfo;
  info.width = 0;
  info.height = 0;
  info.profile = 0;
  ...
  do {
    ...
    hevc.general_constraint_indicator_flags =
      ((hevc.general_constraint_indicator_flags || 0) << 16) |
...
function readAVCSpecificConfig(avcSequenceHeader: Buffer): IAVCInfo | undefined {
...
function getAVCProfileName(info: IAVCInfo) {
  switch (info.profile) {
    case 1:
      return 'Main';
    case 2:
