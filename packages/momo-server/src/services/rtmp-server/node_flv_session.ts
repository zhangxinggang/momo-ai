...
import type {
  ...
  IFlvServer,
  IPublisherSession,
} from '../../types/rtmp-server/flv-session';
...
class NodeFlvSession {
  ...
  playArgs: Record<string, unknown> | null;
  isStarting: boolean;
  isPlaying: boolean;
  isIdling: boolean;
  TAG: string;
  connectCmdObj: Record<string, unknown>;
  startTime: Date;

  constructor(server: IFlvServer, req: IFlvRequest, res: IFlvResponse) {
    this.server = server;
    this.req = req;
    this.res = res;
  ...
  run(): void {
    ...
    console.log(
      `[${this.TAG} connect] id=${this.id} ip=${this.ip} args=${JSON.stringify(urlInfo.query)}`,
    );
  ...
  stop(): void {
    if (this.isStarting) {
      ...
      if (publisherId != null) {
        (context.sessions.get(publisherId) as IPublisherSession).players.delete(this.id);
  ...
  onPlay(): void {
    ...
    if (this.server.rtmpServer.auth !== undefined && this.server.rtmpServer.auth.play) {
      let results = NodeCoreUtils.verifyAuth(
        String(this.playArgs?.sign || ''),
        this.playStreamPath,
        this.server.rtmpServer.auth.secret,
      );
      if (!results) {
        console.log(
          `[${this.TAG} play] Unauthorized. id=${this.id} streamPath=${this.playStreamPath} sign=${String(this.playArgs?.sign || '')}`,
        );
  ...
  onStartPlay(): void {
    let publisherId = context.publishers.get(this.playStreamPath);
    let publisher = context.sessions.get(publisherId) as IPublisherSession;
  ...
  static createFlvTag(packet: IFlvPacketType): Buffer {
