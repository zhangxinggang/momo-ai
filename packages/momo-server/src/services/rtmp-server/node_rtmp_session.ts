...
import type {
  ...
  IInvokeMessage,
  IRtmpPacketType,
} from '../../types/rtmp-server/rtmp-session';
import type { IRtmpServerConfig } from '../../types/runtime-config';
var AV = require('./node_core_av');
var { AUDIO_SOUND_RATE, AUDIO_CODEC_NAME, VIDEO_CODEC_NAME } = require('./node_core_av');
...
var RtmpPacket = {
  create: (fmt = 0, cid = 0): IRtmpPacketType => {
    return {
      header: {
        fmt: fmt,
        cid: cid,
...
class NodeRtmpSession {
  [key: string]: unknown;
  server: { rtmpServer?: IRtmpServerConfig };
  socket: Socket;
  id: string;
  ip: string;
  TAG: string;
  ...
  parserPacket: IRtmpPacketType;
  inPackets: Map<number, IRtmpPacketType>;
  inChunkSize: number;
  outChunkSize: number;
  pingTime: number;
  pingTimeout: number;
  ...
  rtmpGopCacheQueue: Set<Buffer> | null;
  flvGopCacheQueue: Set<Buffer> | null;
  ackSize: number;
  inAckSize: number;
  inLastAck: number;
  appname: string;
  ...
  playArgs: Record<string, unknown>;
  publishStreamId: number;
  publishStreamPath: string;
  publishArgs: Record<string, unknown>;
  players: Set<string>;
  writeBufferQueue: Buffer[];
  connectCmdObj: IInvokeMessage['cmdObj'] | null;
  objectEncoding: number;
  startTime: Date;
  startTimestamp: number;

  constructor(socket: Socket, server: unknown) {
    const serverRef = server as { rtmpServer?: IRtmpServerConfig };
    const rtmpServerConf: Partial<IRtmpServerConfig> = serverRef.rtmpServer || {};
    ...
    this.parserPacket = RtmpPacket.create();
    this.inPackets = new Map();

    this.inChunkSize = RTMP_CHUNK_SIZE;
    this.outChunkSize = rtmpServerConf.chunk_size || RTMP_CHUNK_SIZE;
    this.pingTime =
      (rtmpServerConf.ping || 0) > 0 ? (rtmpServerConf.ping || 0) * 1000 : RTMP_PING_TIME;
    this.pingTimeout =
      (rtmpServerConf.ping_timeout || 0) > 0
        ? (rtmpServerConf.ping_timeout || 0) * 1000
        : RTMP_PING_TIMEOUT;
    ...
    this.gopCacheEnable = Boolean(rtmpServerConf.gop_cache);
  ...
  rtmpChunksCreate(packet) {
    ...
    let headerSize =
      chunkBasicHeader.length + chunkMessageHeader.length + (useExtendedTimestamp ? 4 : 0);
  ...
  sendInvokeMessage(id: number, opt: IAMFMessage): void {
    let packet = RtmpPacket.create();
    packet.header.fmt = RTMP_CHUNK_TYPE_0;
    packet.header.cid = RTMP_CHANNEL_INVOKE;
    packet.header.type = RTMP_TYPE_INVOKE;
    packet.header.stream_id = id;
  ...
  sendDataMessage(opt: IAMFMessage, id: number): void {
    let packet = RtmpPacket.create();
    packet.header.fmt = RTMP_CHUNK_TYPE_0;
    packet.header.cid = RTMP_CHANNEL_DATA;
    packet.header.type = RTMP_TYPE_DATA;
  ...
  sendStatusMessage(id: number, level: string, code: string, description: string): void {
    let opt = {
      cmd: 'onStatus',
      transId: 0,
      cmdObj: null,
    ...
    this.sendInvokeMessage(id, opt);
  }

  sendRtmpSampleAccess(id: number): void {
    let opt = {
      cmd: '|RtmpSampleAccess',
      bool1: false,
      bool2: false,
    };
    this.sendDataMessage(opt, id);
  }
  ...
  onConnect(invokeMessage: IInvokeMessage): void {
    ...
    this.objectEncoding =
      invokeMessage.cmdObj.objectEncoding != null ? invokeMessage.cmdObj.objectEncoding : 0;
    ...
    if (!this.isIPC) {
      console.log(
        `[rtmp connect] id=${this.id} ip=${this.ip} app=${this.appname} args=${JSON.stringify(invokeMessage.cmdObj)}`,
      );
      context.nodeEvent.emit('postConnect', this.id, invokeMessage.cmdObj);
    }
  }

  onCreateStream(invokeMessage: IInvokeMessage): void {
    this.respondCreateStream(invokeMessage.transId);
  }

  onPublish(invokeMessage: IInvokeMessage): void {
    ...
    if (
      this.server.rtmpServer.auth &&
      this.server.rtmpServer.auth.publish &&
      !this.isLocal &&
      !this.isIPC
    ) {
      let results = NodeCoreUtils.verifyAuth(
        ...
        this.publishStreamPath,
        this.server.rtmpServer.auth.secret,
      );
      if (!results) {
        console.log(
    ...
    if (context.publishers.has(this.publishStreamPath)) {
      ...
      }, 200); //200濮ｎ偆顫楅崥搴＄唨閺堫兛绗傞懗钘夌繁閸掍即鐓剁憴鍡涱暥缂傛牜鐖滄穱鈩冧紖閿涘矁绻栭弮璺衡偓娆忓晙閸欐垵鍤禍瀣╂閿涘奔绌舵禍搴ゆ祮閻礁娅掗崑姘灲閺?
    }
  }
  onPlay(invokeMessage: IInvokeMessage): void {
    if (typeof invokeMessage.streamName !== 'string') {
      return;
    }
    ...
    if (
      this.server.rtmpServer.auth &&
      this.server.rtmpServer.auth.play &&
      !this.isLocal &&
      !this.isIPC
    ) {
      let results = NodeCoreUtils.verifyAuth(
  ...
  onPause(invokeMessage: IInvokeMessage): void {
  ...
  onDeleteStream(invokeMessage: IInvokeMessage): void {
