...
module.exports = function (sender: IRtmpPushersSender): void {
  ...
  const pushers: Record<string, { publisher: IRtmpPublisherInfo | null }> = {};
  const sessionMap = (coreCtx as unknown as { sessions: Map<string, IRtmpSessionStat> }).sessions;
  ...
  for (const key in pushers) {
    if (pushers[key].publisher) {
      ...
      result.total++;
      result.rows.push(pushers[key].publisher as IRtmpPublisherInfo);
    }
  }
