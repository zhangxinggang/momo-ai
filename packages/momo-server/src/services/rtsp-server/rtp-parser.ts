import type { IParsedRtpPacket, IPayloadTypeInfo } from '../../types/rtsp-server/rtp';

class EasyRtpParser {
  static FIXED_HEADER_LENGTH: number;
  static payloadTypesHash: Record<number, IPayloadTypeInfo>;

  constructor() {}
  static parseRtpPacket(buf: Buffer): IParsedRtpPacket {
  ...
  static parseRtpPayloadType(payloadType: number): IPayloadTypeInfo | undefined {
  ...
  static isKeyframeStart(rtpRawdata: Buffer): boolean {
    ...
    if (
      rtpRawdata.length >= 2 &&
      rtpRawdata.readUInt8(0) == 0x7c &&
      (rtpRawdata.readUInt8(1) == 0x87 || rtpRawdata.readUInt8(1) == 0x85)
    ) {
      // fu-a
      return true;
    }
