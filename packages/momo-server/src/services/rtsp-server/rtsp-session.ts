...
var rtpParser = require('./rtp-parser');
var BufferPool = require('./buffer-pool');
var sdpParser = require('sdp-transform');
var getPort = require('get-port');
var dgram = require('dgram');

import type { IRtspResponseOptions } from '../../types/rtsp-server/session';
...
class RTSPSession extends event.EventEmitter {
  constructor(socket, server) {
    ...
    this.id = shortid.generate(); // session id
    this.socket = socket;
  ...
  *genHandleData() {
   ...
   */
  makeResponseAndSend(opt: IRtspResponseOptions = {}) {
   ...
   */
  async handleRequest(req: RTSPRequest) {
    console.log(`<<<<<<<<<<< request[${req.method}] <<<<<<<<<<<<<`);
    console.log(req.raw);
    var res: IRtspResponseOptions = {
      method: req.method,
      headers: {
        CSeq: String(req['CSeq'] || ''),
        Session: this.id,
      },
    };
    switch (req.method) {
        ...
        if (mtcp) {
          ...
          if (control == this.aControl) {
            ...
            if (this.type == 'pusher') {
              ...
              tsParts.splice(
                tsParts.indexOf(mudp[0]) + 1,
                0,
                `server_port=${this.aRTPServerPort}-${this.aRTPControlServerPort}`,
              );
              ts = tsParts.join(';');
            }
          }
          if (control == this.vControl) {
            ...
            if (this.type == 'pusher') {
              ...
              tsParts.splice(
                tsParts.indexOf(mudp[0]) + 1,
                0,
                `server_port=${this.vRTPServerPort}-${this.vRTPControlServerPort}`,
              );
              ts = tsParts.join(';');
            }
          }
      ...
      case 'PLAY':
        ...
        res.headers['Range'] = String(req['Range'] || '');
        break;
      case 'RECORD':
        break;
      case 'TEARDOWN':
  ...
  stop() {
    ...
    console.log(`rtsp session[type=${this.type}, path=${this.path}, id=${this.id}] end`);
  }
