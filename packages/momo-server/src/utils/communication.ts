import type { IMailServerConfig } from '../types/runtime-config';
import type { IMailOptions, TSendMailCallback } from '../types/utils/communication';
 ...
 */
const nodemailer: {
  createTransport: (config: IMailServerConfig) => {
    sendMail: (options: IMailOptions, cb: (err: Error | null, info?: unknown) => void) => void;
  };
} = require('nodemailer');

class Communication {
  sendMail(options: IMailOptions, cb?: TSendMailCallback): void {
    ...
    const mailConf = (mailer?.[mailer.server] || {}) as IMailServerConfig;
