...
import type { ILoggerConfig, IMailerConfig, IMailServerConfig } from '../types/runtime-config';
import type { IAppenderConfig, ICategoryConfig, ILoggerRuntimeConfig } from '../types/utils/logger';

class Logger {
  private logger: ILoggerConfig;
  private mailer?: IMailServerConfig & { defaultRecipients?: string[] };

  constructor(config: ILoggerRuntimeConfig) {
    config.logger = config.logger || { start: false, rootDir: '' };
    config.communication = config.communication || {};
    this.logger = config.logger;
    let mailer = (config.communication.mailer || {}) as IMailerConfig & {
      start?: boolean;
      server?: string;
      defaultRecipients?: string[];
    };
    mailer.start &&
      (this.mailer = (mailer[mailer.server || ''] || {}) as IMailServerConfig) &&
  ...
  init(): void {
    ...
    let appenders: Record<string, IAppenderConfig> = {
      stdout: {
        type: 'console',
        // layout: {
        // 	type: 'pattern',
    ...
    let categories: Record<string, ICategoryConfig> = {
      default: {
